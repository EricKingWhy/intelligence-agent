...
**来源须如实区分**：项目级无进展窗口的 4 是**本项目从成熟 stuck 检测设计得出的推断**，
**不是**任何上游的既有默认值——引用该阈值时 MUST NOT 把它说成上游默认（#305 §8 的 provenance 条款）。
其余四个阈值同样以本表为唯一权威，任何重复叙述 MUST 以本表为准。
...
除 local fuse（§5.1 的高位保险丝）之外，本护栏实现更具体的重复调用保护：
