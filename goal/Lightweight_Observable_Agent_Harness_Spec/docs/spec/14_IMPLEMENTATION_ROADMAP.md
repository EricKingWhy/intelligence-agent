...
## 3. 长任务执行边界（#305 冻结契约的依赖指针；**不新增阶段、不建第二套 Roadmap**）

`02 §5.1–§5.4` / `03 §3.4/§5–§7` / `04 §9.1` / `10 §5.1` / `11 §6.1` / `12 §9.1` 记录的长任务
分层预算、同 run 暂停恢复、stuck 检测与完成判定，是 Phase 16 之后一批**依赖有序**的工作
（父规格 `#305`，契约冻结 `#306`，实现 `#307`–`#320`）。依赖边固定为：
...
#314 → #315
#316 → #317
#312, #313, #314, #317, #287 → #318
#313, #315, #316, #317, #318 → #319
#319 → #320
```

本节只钉「谁依赖谁」，不复述交付物、阈值与 Gate——它们是规格章节、ADR-0044 与各 Issue
Verification 的内容。实现的完成判据以对应 Issue 的 Verification 为准；机读进度见
`docs/SDD_TICKET_TRACKER.md`（本目录保持冻结）。
