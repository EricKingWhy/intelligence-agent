...
def zero_content_parent(sha: str, parents: list[str], trees: dict[str, str | None]) -> str | None:
    ...
    ⚠ **本判据只治 GitHub PR 合入那一种形状，别读成"空表问题被根治了"**。2026-09-26 实测本仓区间
    `089524a~1..da7dfef1`（714 笔 commit / 21 个 merge）：树 == 某父的**只有 2 个**，且恰好就是
    两笔 PR merge（`c8e04cb9` / `da7dfef1`）⇒ 对"每个 PR 合入都重现"这条病，覆盖率 **2/2**；另外
    **19 个是取并集形状**（`d37c2f8c8` / `cbe2e0ccb` / `1049a59c1` / `b636a0050` …，多为本地
    `merge origin/main` 集成）—— 它们**至今全靠台账行覆盖**，本判据不放行它们。
    要连它们一起放行，判据得从「树 == 某个**父**的树」放松成「每个 blob 都等于某个**祖先**的
    blob」之类 —— 那是**更弱**的一条（"合并未引入新 blob" ≠ "这个提交零新增内容"），不在本批
    范围内，需单独裁决。
    """
    if not parents:
