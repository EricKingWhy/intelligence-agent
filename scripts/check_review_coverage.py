...
- **自 2026-09-23 起本文件是权威实现**（协议 §8.7 表第 4 行：`.sh` 保持冻结、只对拆分前台账
...
- 末行的归属枚举：`.sh:186` 只列三类（审查行 / 白名单 / 台账记账），本文件列全五类（补上
  docs-only 与零新增内容）—— 纯措辞、不参与判定，改它是因为它列的就是本文件的判据集合。
...
里逐条自校验过 docs-only 的 commit / **恰好只改台账文件本身**的记账提交 / **整棵树逐字等于
某个父提交的树**的零新增内容提交。
...
def zero_content_parent(sha: str, parents: list[str], trees: dict[str, str | None]) -> str | None:
    ...
    是两侧的并集，"零新增"对它不成立。本仓的 PR 流程**不产生**该形状（`strict` ⇒ 合并结果与分支
    顶端逐文件相同）。
    """
    if not parents:
...
def main(argv: list[str]) -> int:
    ...
    print(f"✅ 台账覆盖闸门通过：{base_literal}..HEAD 每条 commit 均有归属"
          f"（审查行 / 白名单 / 台账记账 / docs-only / 零新增内容）。")
    return 0
