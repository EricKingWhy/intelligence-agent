...
class MapRow:
    ...
    __slots__ = ("focused", "gotchas", "lanes", "layer", "neg_tier", "sub_features", "surface")

    def __init__(self, cells: list[str]) -> None:
        (self.surface, self.layer, self.sub_features,
    ...
    with open(p, encoding="utf-8") as fh:
        for lineno, raw in enumerate(fh, 1):
            ...
            if not line.strip() or line.lstrip().startswith("#"):
                continue
            cells = line.split("\t")
            if cells[0] == "surface":       # 表头
                continue
            if len(cells) != 7:
                raise ValueError(f"{MAP_PATH}:{lineno} 列数 {len(cells)} != 7")
            rows.append(MapRow(cells))
    if not rows:
        raise ValueError(f"{MAP_PATH} 没有数据行")
    return rows
...
def changed_files(rev: str) -> list[str] | None:
    ...
    rng = rev if ".." in rev else f"{rev}..HEAD"
...
def build_lanes(since: str) -> list[Lane]:
    ...
    rng = since if ".." in since else f"{since}..HEAD"
...
def main(argv: list[str]) -> int:
    ...
    if affected_rev:
        ...
        if affected["unmapped"]:
            # fail-closed：有路径映射不到 ⇒ 核对不了就不放行，退回**全量**车道（下面只加 focused）。
            pass
        else:
