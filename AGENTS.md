...
**写受控文件的落盘纪律**（2026-09-23 立）：凡把中文 / 长文本写进受控文件，一律"先构好全文 → 脚本落盘再跑"，禁止 `python -c` / `bash -c` 内联承载正文；正文优先用 **JSON 承载**。理由见 `docs/SDD_WORKFLOW_PROTOCOL.md` §8.5 第 5 条（实测事故：内联文本里的反引号被 bash 当命令替换执行、三引号提前闭合导致落盘文件语法错误）。
