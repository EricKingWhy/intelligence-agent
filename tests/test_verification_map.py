...
装饰），`--affected` 就会**静默少跑**——那正是"放松"的形状。所以本测试把四件事钉死：

1. **覆盖面**：`git ls-files` 里的**每一个**被跟踪文件都必须被 ≥1 行匹配（无 catch-all 可依赖）。
   ⇒ 新增一个顶层代码面而不补映射，本测试会红。
2. **结构合法**：7 列、layer 唯一、`lanes` 在词表内、`focused` 路径在盘上存在、`neg_tier` ∈ 1..5、
...
4. **两份实现逐项相同**：本文件自带一个**独立**的匹配器，对每个被跟踪文件与 `scripts/gate0.py` 的
   `affected_summary()` 对账——`map` 与 `--affected` **不是两套真相**。
5. **focused 真的跑得动**：`focused` 指向 pytest 目标时，必须**真能收集到用例**（`tests/**` 目录下
