...
> **历史审计快照（2026-09-15）**：本文里关于“当前 v2”、旧 §16 和跨仓状态的结论只描述当时。流程已更新为 `docs/SDD_WORKFLOW_PROTOCOL.md` V3-lite；本文用于理解历史变更，不是当前执行指令。
...
## E-8（2026-09-16 执行后独立复核：该批改造本身没查出错误，但留了四条待决）

> 用户要求「review 一遍确保万无一失」，于是对 `2c172ca..71ab0a9` 这 7 个 commit
> 做了一次**执行后复核**（不是执行前复测，与 E-1~E-7 的性质不同，故另起一节）。
> 方法：不采信 commit message，全部重新实测。

**结论：该批改造方向正确、执行干净。** 实测证据：

- AGENTS.md 的 13 个、CLAUDE.md 的 16 个 `§引用` **全部解析成功**（CLAUDE.md 的引用全部
  落在 AGENTS.md 的实际章节上）；AGENTS.md 47 个章节**无重号、无同级跳号**；
- `§7` 恰好 **22 条**、编号连续 1..22，与 CLAUDE.md 里写的「22 条架构不变量」一致；
- `§2` 五个 `SPEC_ROOT/` 文件、`§3` 十一个规格文件**全部存在**，无死链；
- 「路径陷阱」声明成立：`docs/spec/` 下 00/01/13/14 确实一个都没有，README.md 是陷阱说明；
- 行尾断言与实测逐字相符（main / backend 工作树 858 行全 CRLF，frontend 858 行全 LF）；
- `§16` 的跨仓断言成立——前端门禁确实在前端仓库自己的协议文件里，命令行逐字一致；
- 该批只碰规则/文档文件（`AGENTS.md` / `CLAUDE.md` / 协议 / tracker / PHASE_STATUS /
  `docs/spec/README.md`），**`web/` 与后端源码零改动**；
- 手册 §7「绝对不要动」清单逐项核对：22 条不变量、§4 审查职责、§9.5 懒惰阶梯、§9.6 八荣八耻、
  §9.2/§9.4 补充、§14.4/14.7/14.8/14.11/14.12、§15 正文、§6 REUSE 枚举、§3 映射关系**全部在位**；
- 第二轮（`71ab0a9`）自述的 12 项修复，磁盘确认 **11 项**（第 12 项见下）。

**本轮修掉的三处小瑕疵**（`AGENTS.md` + 协议）：

| # | 位置 | 问题 | 处置 |
| --- | --- | --- | --- |
| 1 | 协议 §1 流程图 | 图示写 `测试全绿`，与 §1.1 正文的「**门禁全绿**才 commit」不一致（第二轮只改了正文、漏了图示）——同文件内术语漂移 | 改为「门禁全绿」（框宽不变） |
| 2 | `AGENTS.md` §13.4 | 合并流程缺 §14.6 的「先把 main 合回 feature 分支」，也缺指向 §14.6 的引用（即 R2-2 的最低形式；E-4 第 3 条写的"已重写"只部分成立——§13.3 被拆分了，但那一步始终不在流程里） | 流程顶部补 `origin/main → 先把 main 合回你的短分支（§14.6「先回后正」）` |
| 3 | `AGENTS.md` §13.5 | 示例用 `git diff main...feat/backend` / `...feat/frontend`（长命分支名），与 §13.2「不要把某个仓库长期挂在 feature 分支上」读起来矛盾 | 改为占位符 `git diff main...<你当前的短分支>` |

**四条待决项与用户裁决（2026-09-16 用户已拍板，全部关闭）**：

1. **凭证零泄漏红线不在 `AGENTS.md`**（安全相关）。
   该规则目前只存在于 `CLAUDE.md` §3 第 1 条（`.env` 值绝不打印/提交/复制；key 名可列、值不可列）。
   `AGENTS.md` 里与之相关的只有两处**不构成红线**的内容：§4.3 的审查检查项「Secret 泄露；」、
   §13.1 把 `.env` 列进「不要求跨仓库同步」。
   逐 commit 验证：`AGENTS.md` **从未**有过这条（`2c172ca^` … HEAD 全部 0 次命中）——
   **是历史性缺失，不是本批删的**；`CLAUDE.md` 其余 6 条红线都能在 `AGENTS.md` §7/§14 找到对应，
   只有这第 1 条不对称。
   → **裁决：不往 `AGENTS.md` 补**（用户 2026-09-16 原话「既然 `AGENTS.md` 里没有，那就不需要了」）。
   `CLAUDE.md` 里那条**原样保留、一字未改**（用户未要求删除安全红线，不做扩大解释）。本项关闭。
2. **R3-6：`docs/integration/MERGE_EXECUTION_ORDER.md`**（2026-09-10 的自包含执行单）。
   → **裁决：按推荐的"加横幅"方式解决，已执行**。文件顶部加「⚠️ 历史快照，勿照此执行」横幅，
   实测依据：它引用的三个 sha（源 tip `697a085` / 目标 `ebb2d68` / merge-base `c00f742`）
   **现在都是 `main` 的祖先**——那次合并早已执行完毕，按本单施工会去合一个不存在的状态。
   同时就地标注第 4 行「merge 与 push 均需用户明确批准」已被 `AGENTS.md` §14.4 常设授权取代
   （集成与集成后的 `push origin main` 是常设授权；feature 分支 push / PR merge / cherry-pick /
   revert / 冲突后 add / 删分支仍需单独批准）。保留其"跨两个独立仓库执行合并"的模板价值，
   但明确要求每次重新实测、不得复用本单数字。
3. **开放问题 4 / E-6 的收尾**：#210（行尾策略）与 #211（`docs/spec` 改名）两个 issue 仍 OPEN，
   但两者的**决定其实都已发生**（分别是「暂不统一」与「不改名」，缓解措施 `docs/spec/README.md`
   已落地）。→ **仍待用户明示**：本轮**未关单、未改 issue 状态**（按 §14.12，关单要核实实际状态，
   且这两票属"决定已做"而非"交付完成"，判据该用户定）。**这一条是本节唯一仍未闭合的项。**
4. **遗留 worktree / 分支**：backend 仓库仍有 `D:\intelligence-agent-fixbug`（`feat/FIX-test-BUG`）
   与本地分支 `feat/backend`、`feat/FixBUG`；frontend 仓库仍有 `D:\intelligence-agent-frontend-ws6`
   （`feat/frontend-ws6-ws7`）。
   → **裁决：不需要清理（用户 2026-09-16 明确）。** 本项关闭；后续复核不要再把它当"遗留问题"反复报。
