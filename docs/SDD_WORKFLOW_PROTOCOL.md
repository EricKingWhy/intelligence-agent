...
8. **禁止未审查的 commit 进集成**：交付前跑 `scripts/check_review_coverage.sh`（台账
   `docs/review_ledger.tsv`）。它机械地对账「`<最早台账 base>..HEAD` 的每条 commit 是否
   落在某次审查的范围内」，唯一的例外是 `[whitelist]` 段，而**白名单只收 docs-only commit**
   （脚本自己校验改动文件全是文档）——**代码提交永远不能靠白名单放行，只能去补一次审查**。
   为什么需要这条（2026-09-17 实测两个案例，都是复验才发现）：
