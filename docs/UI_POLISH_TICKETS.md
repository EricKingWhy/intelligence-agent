...
- [ ] 头标显示 `2 runs · 19 事件`，不再显示裸 run_id。
  - **2026-09-16 批注（有意偏离，勿当回归报）**：窄面板容器查询（Inspector 内容宽 ≤360px，
    **含默认宽度 340**）下 `.detail-run-id` 整体 `display:none`，即**默认宽就看不到这条范围**。
    原因：340 + 一个长档位名（`agent_profile=research_review` 需 126px）下，该行的全部需宽项
    （状态徽标 62 + 档位 126 + 范围 94 + 三个控制按钮 73 + 标题 87）无法共存，必须有取舍；
    而这条范围在头部是**最会把整行撑爆**的一项（实测被压成 30px×90px 的竖条并把关闭按钮
    顶出面板 44px）。取舍方向：状态 > 档位 > 控制 > **范围（窄宽退出）**；范围数据在
    **Timeline 页签**可读（分组头条数 + `.detail-tab-count` 事件数），**Overview 页签里没有**
    这两个数。方向已由 `e2e/y-inspector-peek.spec.ts` AC8 锁死（320 隐藏 / 480 完整可见），
    所以本 AC 应读作「**宽面板下**头标显示 `2 runs · 19 事件`」。`g-visual-qa.spec.ts` 那条
    `toHaveText` 对隐藏元素照样通过，抓不到这条偏离（这正是补 AC8 边界的理由）。
