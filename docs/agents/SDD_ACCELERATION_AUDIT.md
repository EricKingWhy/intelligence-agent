...
> **状态（2026-09-23 · 本文件第四次按实测改写；早先状态块已过期）**：**三批（批 1 / 批 2 / 批 3）全部完成并已推送**。
> **批 1**（协议机制修订）已推送；**批 2**（闸门双实现 + pre-push Gate-0）完整链条走完并已推送；
> **批 3**（`#293` = 闸门本体：台账拆 `docs/review_ledger.d/` + 双读兼容 + `docs/gate/*.json` 机器读数 + 三条 P1 修复）已收尾、已推送、已关单。
> **推送结果**：`origin/main` = `015c41f`，与本地 `refs/heads/main` 一致；fast-forward 两段
> `f0c7c4b..e89eed5`（10 笔）→ `e89eed5..015c41f`（2 笔 docs-only）。
> **闸门现状 = GREEN**：`python scripts/check_review_coverage.py` **exit 0**，
> **527 提交 / 已审查 369 / 待判定 158** 全归属（`089524a~1..HEAD` 每条 commit 均有归属）；pre-push `Gate-0` 6/6 PASS（墙钟 14.0s）；`git diff --check` RC=0。
> **本线新增**：协议新增 **§9「阶段 → 外部 Skill 引用」** + 搬运 pstack skill 到 `docs/agents/skills/`
> （最终 **7 个**，逐字节，MIT）——见 **§10**；该批 **已完成两轴独立补审、6 条 P2 全数处置**——见 **§10.8**。后续**窄复验（主会话内联、非独立）**：8 项机械复算 **15/16 PASS**，唯一 finding（§5 扫描口径一处引号）已更正——见 **§10.8.1**。
...
- 口径**逐条对齐** `.sh`：最早 `base` 取法（`base` 保持台账里的**字面量**，故末行可能打印 `089524a~1`
  ...
  ⚠ **此后又新增两条 `.sh` 里没有的判据**（2026-09-23 的 docs-only 按路径自动归属、2026-09-26 的
  「零新增内容」）⇒ 上面这份清单是**当时的快照**，**不再等于当前判定集**；差异与理由见
  `check_review_coverage.py` 的「与 `.sh` 的已知（且无害）差异」段与 §9.4.4。
- **`.sh` 是语义参考、冻结**：协议按**行号**引用它的 `DOC_PATTERN`（`:51`）与 fail-closed 形状
  （`:74-75`）⇒ **不得**把它改成包装脚本、**不得**重排它的行。双实现的一致性靠"同 tip 同结论"守
  —— ⚠ **但这条只在 `docs/review_ledger.d/` 为空（拆分前的单文件布局）时可比**：`.sh` 只读旧单文件，
  台账已迁入目录 ⇒ 它在当前布局下**跑不到判定循环**（实测 `exit 1`、只输出「台账里没有审查行」）。
  该边界与 `.sh` 的归宿（删除 / 改写为历史制品）登记在 §9.4.1，**待用户裁决**。
...
| 6 | 两侧无 CI | 无远端强制 | ~~本线**不**引入 CI（需求方未要求；pre-push 只是本地替代，且只在本 clone 生效）~~ ⇒ **2026-09-26 更正**：服务端**已**引入 CI —— `.github/workflows/gate0.yml` 把同一套 6 条机械车道挂成 `main` 的**必需状态检查** `gate0`（`enforce_admins` 为真、无旁路主体，直推被 `GH006` 拒）。本行原结论只对**当时**（2026-09-25 前后）成立；「无远端强制」这一性质已消失 |
...
> **⚠ 2026-09-22 追加（批 3 / `#293`）：上表的成立范围已被本次改动**缩小**到"拆分前的台账布局"。**
> 台账审查行已按票面要求拆到 `docs/review_ledger.d/`（一文件一条，旧单文件只剩契约头 + `[whitelist]`
> 段），而 `.sh` 是**冻结**的、只认单文件 ⇒ 实测在 `0896336` 上跑 `.sh` 直接 `exit 1`、输出只有
> `台账里没有审查行`。**无假绿风险（fail-closed）**，但"迁入目录后比判定集"这句话**不可执行**了。
> 处置：本节表格与结论**不改**（它们是对 `6b95cef2da69` 那棵树的真实历史读数），只在**三处**
> （本注、`AGENTS.md` §14.10、协议 §8.8.5）登记这个边界；`.sh` 该删除还是改写为历史制品，
> **留待用户裁决**。
...
#### 9.4.4 覆盖闸门第四条判据「零新增内容」（2026-09-26）

**症状（实测两例，非推断）**：`main` 顶端的覆盖闸门报 `❌ 未审查且未声明` 指向刚合入的 PR merge
—— PR #321 的 `c8e04cb9`、PR #322 的 `da7dfef1`（前者处置落地**当天**再复现）。两笔的
`git show --cc --name-only` **都是空的**。

**根因（结构性）**：`strict: true` 要求分支与 `main` 同步，而 `rebase` 属默认禁止项（且已被分支
保护关掉）⇒ 只能先把 `main` 并进分支 ⇒ GitHub 合的这次 merge 与**父提交二（分支顶端）逐文件相同**
⇒ `--cc` 无内容可列。而空表在本闸门里的默认语义是「核对不了 ⇒ fail-closed」（枚举失败也是空表），
白名单那一支对空表**同样** fail-closed ⇒ **每个 PR 合入都会在 `main` 顶端重造一个 ❌**。

**判据**：提交的整棵树**逐字等于某个父提交的树** ⇒ 零新增内容 ⇒ 自动归属（`zero_content_parent`
/ `format_zero_content`，打印树与父的**完整** sha 供 `git diff --name-only <父> <sha>` 复算）。
**它不是放松**：说"空表就放行"是拔掉 fail-closed；说"树逐字等于某个父"是**正向证明**，且要求
`--cc` 文件表与 tree 对象**两条互相独立的 git 通路**得出同一结论才放行 —— 与方案 C 用"路径客观
事实"取代"作者声明"同源。

**证据**：`tests/tooling/test_review_coverage_zero_content.py`（7 条判据层 + 2 条合成仓库端到端）。
三条变异实测均被**恰好预期**的测试抓住：拆掉接线 ⇒ **只**红正控端到端（其余 8 条全绿，故"判据层
测试看不见接线"是实测）；判据偷懒成"空表即放行" ⇒ 红 4 条含反控端到端；去掉 `None == None` 守卫
⇒ 只红那条。真实历史侧的**读数（别混为一谈）**。⚠ 计数口径先说清：`已审查` = 被台账区间**覆盖**的提交数，`待判定` = 不在任何台账区间内、需逐条判定的提交数 —— 后者**不等于“未归属”**（其中绝大多数按 docs-only / 台账自身更新**自动归属**为 ✅，只有少数报 ❌）：

- **修前**（`9367a57e` 之前的闸门）⇒ `❌ 未审查且未声明: da7dfef1` / `exit 1`；
- **修后、补丁生效而本批 code commit 尚不存在时**（补丁在工作树、未提交）⇒
  `✅ 零新增内容（树 7b8935ca… == 父 0b6272e3… 的树）: da7dfef1 …` / **`exit 0`**；
- **修后、在已提交本批 code commit 的 HEAD（`9367a57e`）上跑** ⇒ 唯一 `❌` 就是 `9367a57e` **自己**
  （它的审查行尚未落盘），而 `da7dfef1` 那行如实 `✅` / `exit 1`。

- **本批 4 笔审查行已进台账、且台账落点笔已提交之后**（HEAD = `4ee53f45`，区间 `09ca47a1..HEAD`）⇒ 提交总数 **718** / 已审查 **469** / 待判定 **249** / **`exit 0`**、**0 条 `❌`**。⚠ 本节四处读数**各属不同 HEAD**，别混：`9367a57e` 那次（上一条）唯一 ❌ 是本批第 1 笔自己；行已写、未提交时（HEAD 仍 `3bd597be`）是 717 / **469** / 248 / `exit 0`；落点笔提交后又 +1 笔 docs（它按 docs-only 自动归属）⇒ 718 / 469 / 249。

⚠ 中间那条 `exit 0` 的取值条件是**补丁在树、本批 code commit 未提交**；**不能**读成"本批在 HEAD
上绿了"—— 本批要等自己的审查行进台账才会绿，那正是"每条 code commit 都需要一条真实 review 行"的
**正常**状态，与本判据无关。

**残余（刻意保留）**：文件表为空**且树不等于任何父**的 merge（两侧各改不同文件、结果取并集 ⇒
每个改动文件都等于某个父的 blob）**仍 fail-closed**。⚠ **2026-09-26 用闸门自己的 `files_of` 实测**
（区间 `089524a~1..da7dfef1`，**713** 笔 commit / **21** 个 merge），21 个 merge 分四类：

| 类 | 数 | 谁覆盖它 |
|---|---|---|
| 树 == 某父 | **2**（`c8e04cb9` / `da7dfef1`，恰好两笔 PR 合并） | 本条判据（对那条病 **2/2**） |
| 文件表为空**且**树 ≠ 任何父 | **3**（`cbe2e0ccb` / `2fb974ff8` / `cbe3b09e9`） | **无人** —— 靠台账行，即上面那条残余 |
| 文件表非空且全是文档 | **12** | **另一条**机械判据（docs-only 按路径自动归属） |
| 文件表非空且含代码 | **4**（`1049a59c1` / `b636a0050` / `3b4e93c6f` / `080cc1464`） | **真实审查行** |

⇒ 别把"树 ≠ 某父"（共 19 个 = 3 + 12 + 4）当成残余：其中 **16 个今天已被别的路径机械覆盖**。
把本判据推广到那 3 个需要一条**更弱**的规则（"合并未引入新 blob" ≠ "零新增内容"），属**另案**、
需单独裁决，不在本条范围内。

**顺带发现：`[whitelist]` 段已成空放行路径（2026-09-26）**：`is_docs_only(files)` 为假 ⇒ `files` 里
**必有**非文档文件 ⇒ 白名单那一支只能落到两个**失败**分支（文件表为空 ⇒"核对不了"；或 `bad` 非空
⇒"改了非文档文件"）⇒ 脚本里那句 `✅ 白名单(docs-only)` **不可达**，现有 `[whitelist]` 条目此后唯一
的作用是产出"未被用到"告警（本仓 131 条）。该结论由两轴 Standards 轴提出、作者**逐条复核源码**确认。
本批**不动它的代码**（去留属**用户裁决项**，与 §9.4.1 的 `.sh` 归宿同批待决），只把口径改成准确表述：
协议 §7 第 8 条原写"白名单行仍被认，只是冗余"——**与代码可观测行为不符**，已改。

**独立佐证（不只有结构证明）**：`docs/phase_status/2026-09.md:803` 那份 2026-09-25 的实测读数里，
归属性条数已明写**「白名单（手写 docs-only 声明）**0**」**（另两项：docs-only 自动归属 89 / 台账
自身更新 101）—— 即方案 C 落地后白名单**一次都没被用上**，与上面的结构证明同向。

**口径漂移（登记，不回改）**：末行的归属枚举去掉了白名单 ⇒ **七处带日期的历史引文**仍抄着旧枚举
（`docs/SDD_TICKET_TRACKER.md:4747` / `:4813` / `:4885`、`docs/phase_status/2026-09.md:589` / `:803` /
`:808`、`docs/agents/SDD_V31_LIVE_VALIDATION_286.md:73`），另加 `.sh:186` 的实现本身。这些引文是
**当时实测输出的原文**，**不回改**（回改等于篡改读数记录）；`.sh` 冻结不动。将来按它们 grep 当前
输出会 grep 不到 —— 那是口径已变，不是抄错。

**本批两轴独立审查（2026-09-26）**：P0/P1 = **0**。

- **第一轮**（审 `9367a57e`）：Standards 轴 1×P2 = 上面那条白名单口径措辞（已改）；Correctness 轴
  1×P2 = 原稿写"本仓 PR 流程**不产生**该形状"**属误**，已改；两轴另各出 P3，其中一条（删掉
  `expect.match(line)` 的形状校验，无测试能抓）已补测试（见用例文件"变异实测"段），其余登记为良性
  或未被证伪。
- **第二轮（修后重审，范围 `9367a57e..d3c9373d`）**：又出 2×P2，且都长在**第一轮改写的产物**上 ——
  ① `FAIL_HELP`（闸门失败时打印的那段帮助）**仍在教用户写 `[whitelist]`**：第一轮把协议与 docstring
  都改了，却漏了**失败用户唯一一定会读到的那段字符串**，而那一支早已不可达 ⇒ 照做只会白写一行、
  再收一个"未被用到"告警；② 第一轮改写的**实测数字本身不准**：写"19 个取并集、全靠台账行覆盖"，
  而 19 是"树 ≠ 某父"的总数，真正的残余（文件表为空**且**树 ≠ 任何父）**只有 3 个**（见上表），
  且两个点名例子（`d37c2f8c8` / `b636a0050`）根本不在那一类里。两条均已在第三笔提交里订正。
  另有 1×P3：新用例 `test_resolve_trees_rejects_a_line_that_is_not_a_tree` 依赖"闸门所在目录是个
  仓库"，而变异钩子 `WBI_GATE_UNDER_TEST` 正是把闸门复制到**仓库之外**跑 ⇒ 该用例会以一个与判据
  无关的理由红；已改成**自建临时仓库 + `monkeypatch` 指 `REPO_ROOT`**。Standards 轴的 1×P3（七处
  带日期的旧枚举引文）判定为**同意不回改**。
  ⚠ **教训（留在这里防复犯）**：这两条 P2 分别是"改了 A 处、漏了 B 处"与"把两个不同的集合当成
  同一个"。前者靠**grep 全部同源字符串**防（`FAIL_HELP` 与协议/docstring 是同源文案），后者靠
  **用被测代码自己的函数去测**防 —— 本轮订正就是把分类换成闸门自己的 `files_of`，而不是另写一套
  近似的 git 命令（第一轮正是栽在这个近似上：量的是"树 ≠ 某父"，说的是"空表"）。

**审查过程事故（如实记录）**：Correctness 轴那个只读子代理做合成仓库探针时，一次失败的 `mkdir`
把它的 shell 留在**另一个 clone**（`D:\intelligence-agent-backend`，T313 的施工 clone）里，于是它的
`git init` 成了既存仓库的空操作，随后的 `c0/c1/c2` 三笔与一次 merge **落进了那个 clone**（还临时
推进了 `zcode/T313-model-request-budget`、新造了本地 `main` / `side` 分支）。它自查后用
`git reset` 把该分支退回 `02daddd`（= 探针前的原值，同一对象）并删掉探针分支；**受跟踪的代码与提交
零损失**（`git status` 干净、无改动文件）。⚠ 但那一步清理**连带删掉了该 clone 根下约 25 个未跟踪的
`.tmp_*` 探针脚本/日志与 `.zcodeignore`**（`git clean` 语义；仓库根目录与 `.git/logs/HEAD` 的 mtime
都停在探针清理那一刻 `01:11`、此后再无变化，故可定位到那一步）。它们未进版本控制 ⇒ **不可恢复**，
但属 T313 会话的一次性草稿，非交付物。⇒ **红线补强**：给只读子代理派活时必须显式写"**你的 cwd 是
A 仓库；禁止在 B 仓库执行任何写操作，包括 `git clean`/`reset`/分支操作**"，并核对它的第一条命令
落在哪个仓库。

**`.sh` 不同步（用户 2026-09-26 裁决）**：本判据（与方案 C 的 docs-only 自动归属一样）**只进
`.py`**，冻结的 `scripts/check_review_coverage.sh` **一字不动** ⇒ 无需重编号任何行号引用。
`.py` 里那句"改动任一侧的语义时另一侧必须同步"**已作废**（它写于 2026-09-22，早于 9.4.1 的冻结）。
`.sh` 是删除还是改写为历史制品，仍**留待用户裁决**（与 9.4.1 同一条待决项）。
...
## 10.8.1 窄复验（2026-09-22 深夜；主会话内联、非独立）
...
1. **§6 表 12 行逐项 = blob 实算**（对象 sha1 前缀 / 入库字节 / 入库 sha256 / 本机检出字节）✓；
   **12 个对象 sha1 与上游 `53e579f1` 逐条全等** ✓（与 §10.8 独立复核的 GitHub trees API 口径互证）。
2. **§5 计数复跑**：上游 122 文件 / **29 / 78 / 69** ✓；本目录 12 / **4 / 4 / 4** ✓；第 4 处 =
   `create-verification-skill/SKILL.md:17` 的 `curl-able` ✓；其余 3 处 = `127.0.0.1:4173` ✓。
   **但发现一处口径失真**：§5 框里 `rm -rf` 写成了带双引号字面量，按字面复跑 = 28 / 77 / 68
   （差 `poteto-mode/playbooks/worktree-cleanup.md:9` 一处**裸** `rm -rf`）⇒ 本笔已更正
   （PROVENANCE §5「口径更正之二」），数字不变。诊断：`%TEMP%\wbi_diag2a.py`（四个引号变体逐一
   对比，唯 `rm -rf` 去引号变体 = 29 / 78 / 69）。
3. **宿主 tdd 字节**：codex 3549 B / LF（CRLF 计数 0）、zcode 3587 B / CRLF（38 行）；归一化后同
   sha256，前 8 位 `cb01f66b` ✓。
4. **上游 47 skill / 46 带 `disable-model-invocation` / 唯一例外 `setup-pstack`** ✓（`re.M` 匹配）。
5. **§4.2 九处 `file:line` 引用逐条核**（行内含对应关键词）✓。
6. **审计包 lone CR**：CR 计数 = 1，与 `29cb508` 版本该 CR 前后各 60 字节上下文**全等** ✓；
   无哨兵 `\x01` 残留 ✓。
7. **头部数字**：`22aa291..5c22f777` = **24 笔** ✓；gate 在 `5c22f777` 干净 clone 上复现
   **475 / 331 / 144、exit 1、❌ 3 = `cd02c61` / `29cb508` / `5c22f77`** —— 与头部登记逐项一致 ✓。
8. **旧值残留**：`3541` / `60 处` / `4066` 在 `docs/` + 根级 `.md` 的全部命中均位于「引用旧值并
   声明更正」的合法语境（§10.8 更正表自身 + §6 表的检出字节列）✓。
