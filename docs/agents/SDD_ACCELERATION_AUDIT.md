...
**证据**：`tests/tooling/test_review_coverage_zero_content.py`（6 条纯函数 + 2 条合成仓库端到端）。
三条变异实测均被**恰好预期**的测试抓住：拆掉接线 ⇒ **只**红正控端到端（7 条纯函数全绿，故"纯函数
测试看不见接线"是实测）；判据偷懒成"空表即放行" ⇒ 红 4 条含反控端到端；去掉 `None == None` 守卫
⇒ 只红那条。真实历史侧：修前 `❌ da7dfef1` / `exit 1`，修后 `✅ 零新增内容（树 7b8935ca… ==
父 0b6272e3… 的树）` / `exit 0`。

**残余（刻意保留）**：文件表为空**且树不等于任何父**的 merge（两侧各改不同文件、结果取并集）
**仍 fail-closed**；本仓 PR 流程不产生该形状（`strict` ⇒ 合并结果与分支顶端逐文件相同）。
