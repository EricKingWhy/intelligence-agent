...
每个改动文件都等于某个父的 blob）**仍 fail-closed**。⚠ **2026-09-26 实测：它不是罕见形状，而是本仓
该窗口内的主流形状** —— 区间 `089524a~1..da7dfef1`（714 笔 commit / 21 个 merge）里树等于某父的
**只有 2 个**（恰好就是两笔 GitHub PR 合并），另外 **19 个都取并集**（多为本地 `merge origin/main`
集成）⇒ 它们**至今全靠台账行覆盖**。把本判据推广到它们需要一条**更弱**的规则（"每个改动文件都等于
某个父的 blob" ≠ "零新增内容"），属**另案**、需单独裁决，不在本条范围内。
...
**本批两轴独立审查（2026-09-26）**：P0/P1 = **0**。Standards 轴 1×P2 = 上面那条白名单口径措辞（已按
上文改）；Correctness 轴 1×P2 = 原稿写"本仓 PR 流程**不产生**该形状"**属误**（实测 19/21 取并集，
见上），已按实测改写；两轴另各出 P3，其中一条（删掉 `expect.match(line)` 的形状校验，无测试能抓）
已补测试（见用例文件"变异实测"段），其余登记为良性或未被证伪。
