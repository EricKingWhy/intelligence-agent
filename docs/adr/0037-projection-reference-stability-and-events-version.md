...
**Date**: 2026-09-18
**Related**: ADR-0016 §2（append-only 共享簿记第二例外）——**本 ADR 是它的补充，不覆盖它**；
...
| 基准 | 改造前 | 改造后 |
| --- | --- | --- |
| `applyEvent` @20k 事件 | 240.9 µs/事件 | **<10 µs/事件**（同次报告另测 0.2 µs/事件） |
...
- `web/src/lib/projection.ts:1174` 有**去重短路**
  ...
  重复投递被整帧丢弃、**不 push**，长度自然不变，这种情形下长度作键**恰好**是错的（它显得"没变"）。
...
**(2) 未来一定会困惑**：`ADR-0016 §2` 已经把这个数组写成
「**append-only 共享簿记第二例外**：…`seenSeqs`…与 `events` 数组同构——COW 纪律的两处文档化例外」。
一个读者同时看到「**例外**」和「**版本号**」，会合理地推断其中一个是多余的、可以删掉。
没有这份 ADR，**下一次重构极可能把 `eventsVersion` 当死代码删掉**——删掉之后所有 `memo` 静默失效，
而**测试很可能仍然全绿**（陈旧渲染不抛异常）。

**(3) 真实权衡**：候选方案**有且只有三个**（见 `Alternatives considered`），其中只有一个是可接受的，
...
| | 记录什么 | 状态 |
| --- | --- | --- |
| `ADR-0016 §2` | `events` 数组是 COW 纪律的**第二处文档化例外**（另一处是 `seenSeqs`）：`events` 与 `seenSeqs` 同构，都是**追加型共享簿记**，不参与「被改写才深克隆」的规则 | **仍然有效，本 ADR 不改它、不覆盖它** |
...
| 方案 | 为什么没选 |
| --- | --- |
| **① 把 `events` 改回「每次换引用」**（消费端继续依赖引用相等） | 这是**回退 P0-1**：`3344e34` 的实测（applyEvent @20k 240.9µs → <10µs；`projectHistory` 4650 事件 35.0ms → 2.8ms；20k 事件 1536.9ms → 7.2ms）全部退回。而且 `web/src/lib/projection.test.ts` 的 7 个引用稳定契约测试正是为守住它而设——选这条路＝**为了让 memo 生效而牺牲 100× 量级的性能**，把"陈旧渲染"换成"整体变慢"，问题并没解决。 |
