...
3. **取代事实不可能被静默跳过**（决定性实测结论）：客户端续传游标是**它自己的水位线**
   （"≤ 此值都已应用"；run 启动时取本地对话 `max seq`，重试复用该值），所以它缺的任何事件其
...
**锁定方式**：`web/e2e/supersede-gap-recovery.spec.ts` 是本决议的证伪点。它造出最坏形态——
**取代帧本身在 live 通道被丢掉**，客户端只看到一条跳号的尾帧（与"尾部增长"同形）——断言：
① 任何一次续传游标都不得越过丢帧处（重放窗口必含该帧）；② 旧轮问 + 答整段消失、carrier 与其
回答各恰好一次；③ 该帧确实进了本地事件日志（Inspector 可见）。红证：把该帧同时从所有通道
剔除后旧轮留在视图（`原始问题` 计数 1）⇒ 上述断言有判别力，不是"视图恰好这么渲染"。
