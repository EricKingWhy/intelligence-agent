...
**可读判据**（"家在哪"写进测试），不是静默风险防线：模块真不在了，import 它的文件会当场全断
（`55f2deb` 搬迁前 11 条 / 8 文件；搬迁后测试侧四条并一条 ⇒ `848586e` 上 8 条 / 8 文件——
两个口径都写出来，免得同一份文档里两种读数打架）。
第 15 条只证了它的第二条断言。
...
### 5.2 本批的最终读数与审查轮次（2026-09-22）

**三棵树的读数（同一命令，逐棵可复算；日志在 `.workbuddy/full_248_r1r2*.log`）**：

- `848586e`（**待集成树** = 首版实现 + 两轮 findings 处置）：**3093 passed / 2 skipped /
  42 deselected / 0 failed in 487.24 s**；
- `a54c13a`（findings 处置轮）：**3092 passed / 2 skipped / 42 deselected / 0 failed in 581.93 s**；
- 首版实现（含本批最早那版守卫，日志只留了尾部 20 行）：**3090 passed / 1 failed in 617.23 s**，
  唯一失败即上面那条既有 flake。

3093 = 3092 + 1（N6 新增的精度用例）；3092 = 3091 + 1（P2-1 把一条守卫拆成"三文件扫描 + 四种
写法"两条）——收集数逐棵对得上，别把某棵的数字安到另一棵上。

**独立审查（三轮，均为独立只读子代理；findings 全部带可复跑证据）**：

- **R1** 审 `8310e62`（base `55f2deb`）判"**可集成**"：2×P2 + 4×P3（P2-1 = 守卫可被三种等价
  import 写法绕过；P2-2 = 当前态文档未跟上），处置于 `a54c13a`，红证 20/21 即其证据；
- **R2** 修后窄复验 `a54c13a` 判"**修复成立、可集成**"：逐条复算了两处订正后的计数、独立证明
  四种写法全被拦住、验证三份域文件都进扫描；另出 N1–N7（P3/P4，无阻断），处置于 `848586e`；
- **R3** 窄复验 `848586e` 判"**N1–N7 全部真闭合、可集成**"（无 P0/P1/P2），仅余 1×P3（纯文档
  口径：`:356` 那条"11 条 import"是搬迁前读数 ⇒ 本段上方已按两态改写）+ 1×P4（ADR-0030 那句
  "搬迁前也在 `:228`"对更早一版不成立 ⇒ 已改写为"最近若干版 / 更早一版 `:225`"）。
  **收口到此为止**：R3 之后只做文档口径订正，不再开新一轮（findings 逐条闭合、无新机制）。

**覆盖闸门**：本批三笔（`8310e62` / `a54c13a` / `848586e`）的审查行与记账笔的 docs-only 白名单
唯一住在 `docs/review_ledger.tsv`；闸门退出码与区间见 `docs/SDD_TICKET_TRACKER.md` B-26 段的集成记录。
