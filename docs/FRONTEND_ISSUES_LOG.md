...
## 用户报障批次（2026-09-14）：composer 停止提示 + 用户消息动作行（**仅登记，未修**）
...
**待定**：形态选"按钮左侧内联 `Esc` 提示"还是"删掉文字只留按钮 tooltip"（issue 里已列）。

**验收要点**：流式中左下档位行与提示 `getBoundingClientRect` **不相交**；提示与停止按钮同侧；
`Esc` 中断行为不变（全局键绑定在 App）；非流式不渲染。
...
**待裁决（阻塞实现）**：编辑后发送在**只增不改**的事件模型下走哪条路线 ——
(a) 分叉式（`fork(anchor)` → child 发改后文本；**无需后端改动**，历史零改动，结果落在新会话；
依据 `web/lineage.py:43-49`「锚点消息不进 child」）；
(b) 原地截断（需新后端参数 + 与 spec 03 append-only 硬约束冲突，要 ADR 裁决）；
(c) 只允许编辑最后一条（等价重发）。
次要待定：提示文案「在此次/从此处分叉新会话」择一；三件套是否只给用户消息；是否加时间戳。

**影响面（改动时会踩到的断言）**：`Conversation.test.tsx:292-336`（断言 `.fork-btn` 与文字）、
`web/e2e/b-fork.spec.ts:79,108,118-121`（按 `.fork-btn` 定位 + 逐字断言 title）。

**状态**：两条都**未修**，等用户对上面待裁决项拍板；第二轮报障（用户将另发）一并收编后再排期。
