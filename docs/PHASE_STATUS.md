...
**Phase 8 已完成（MCP Client Capability，ADR-0012 + tickets #63-#68）**：SDK 2.1.1 client + discovery + MCPToolAdapter + 权限映射，双 Gate 达成（统一 Executor 零旁路 / 无双重 retry），fake server 全程确定性测试。下一 Phase 为 Phase 9 前端 Gap 或 Phase 10 后续集成。：seam 泛化（Registry/Descriptor/Error 词汇表/降级三分类）+ plugin config + SKILL.md 渐进披露 + demo capability Gate 双证明 + 双轴 code-review 修复（`984f4c9`）。离线全量 585 passed、8 skipped、8 deselected，ruff clean。证据：docs/PHASE7_GATE.md。下一 Phase 为 Phase 8（MCP）。
