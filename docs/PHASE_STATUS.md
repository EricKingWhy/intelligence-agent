...
- 2026-09-17（对照）：**DSH 设计对照审计**——12 条可借模式逐条对我们实测（多已具备，2 条刻意偏离，2 条真实差异：请求侧模型名不落事件、其余错误家族无机读码）；初稿两条「缺口」被实测推翻后改写。产出 `docs/RESEARCH_DSH_PATTERN_GAP_AUDIT.md`，两条差异**只登记待开票**
