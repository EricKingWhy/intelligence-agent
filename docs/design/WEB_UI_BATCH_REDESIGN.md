...
- 每行 = 图标槽（20px 固定宽，保证多行对齐）+ 标题（主）+ 描述（次级 2 行内截断）+ 右侧选中标记。
  - **已落地（2026-09-17，#214 后为终态）**：槽**恒渲染**（`.picker-item-icon`，没有图标的行也占 20px，否则标题左边界会随图标有无跳动）。
  - **字形由条目声明的 `icon` 名决定**（#214）：三个目录端点（`/api/permission-modes`、`/api/reasoning-efforts`、`/api/agent-profiles`）在内置条目上下发 `icon` 键，取值集是**稳定语义名**（后端 `web/app.py::CATALOG_ICON_NAMES` = `lock`/`pencil`/`unlock`/`layers`/`code`/`search`/`bolt`/`gauge`/`telescope`），前端 `lib/catalogIcons.ts` 把**已知名**映射成字形。名是语义（"只读"用锁），不是字形描述——换字形不改名。**前端不再按 `id` 猜**：`id` 空间是可扩展的，猜出来的字形等于宣称一个后端没说的语义。
  - **未知名 / 缺 `icon` 键 → 留空槽，不编字形**：后端可以先行加名（前端还没实现对应字形），**未升级的后端**（#214 之前那版）压根不带该键——两种情况都渲染空槽，槽仍在所以对齐不破。给未知名编一个字形、或回头拿 id 兜底，都是被禁的「编占位」（PRODUCT.md 原则 3）。判定证据在 `e2e/control-row.spec.ts`：同一目录里 `main` 与 `read-only`（**两者都在被删掉的旧 id 映射表里**）并排——前者声明了名 → 有字形，后者缺键 → 空槽；差异只可能来自键在不在。⚠ 该夹具里的"缺键"是**旧后端**的载荷形态：#214 之后这三个端点**恒发**该键（见下条），所以真后端不会出现"有这个 provider、却没这个键"。
  - **契约口径**：`icon` 键**恒在**（未声明时为 `null`，`app.py` 用 `desc.get("icon")` 无条件下发）；前端窄化只认非空字符串（`api.ts::parseIconName`），其余一律当"后端没说"（"缺键"这一支只服务未升级后端）。⚠ `parseCatalogEntries` 是**白名单投影**——新增契约字段必须在那里登记，否则静默丢掉（#214 落地时实测撞到：后端配好、夹具也带，界面就是不显示）。
  - **名集是跨语言契约，两侧由测试对账**（#217）：两份名单必须同集——后端 `web/app.py::CATALOG_ICON_NAMES` 与前端 `lib/catalogIcons.ts` 的 `CATALOG_ICON_NAMES` 字面量。两道机械闸门：①`ICONS` 的类型是 `Record<CatalogIconName, LucideIcon>` ⇒「名单里有名却没配字形」在 `tsc` 阶段就红；②`tests/web/test_web_phase5_staged_endpoints.py::TestCatalogIcons::test_frontend_mirror_is_in_sync_with_backend_set` 直接读前端那份字面量与后端集合逐值对账 ⇒ **单边增删名必红**（增删名要同一个提交里改两端）。#214 之前真正开着的是**前端侧**（后端侧已被"集合 == 实际下发的并集"与逐 id 映射挡住，实测两态皆红）：前端那把锁比的是自己那份镜像字面量，所以"前端删掉一个后端仍在下发的字形、并同步改掉镜像"⇒ 两套测试全绿、界面上那一行静默空槽（用户可见的退化）——②补的正是这个方向。
  - **本部署可达性**：内置条目恒带 `icon`，所以三条 picker 的**目录条目行**都有字形（首行「默认（未选）」不是目录条目，它一直是无字形的空槽）；空槽只出现在"后端扩展/部署自定义且没声明名"的条目上（夹具用 `longCatalog('mode', 6)` 与显式 `icon: 'sparkles'` 覆盖这两支）。四个调用点都要传 `catalogIcon`（`Composer.tsx` 三处 + `StartTaskInProjectDialog.tsx` 一处），漏传就是静默空槽——按调用点分两处锁：`e2e/control-row.spec.ts` 的 AC4 用例逐条断言 **Composer 的三个** picker，`e2e/u-project-task.spec.ts` 的 AC10 用例断言**弹窗**那条（正是本批 REVIEW 抓到漏传的那一处；两处都实测过"删掉 `catalogIcon` 即红"）。
