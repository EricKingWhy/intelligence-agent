...
- 每行 = 图标槽（20px 固定宽，保证多行对齐）+ 标题（主）+ 描述（次级 2 行内截断）+ 右侧选中标记。
  ...
  - **未知名 / 缺 `icon` 键 → 留空槽，不编字形**：后端可以先行加名（前端还没实现对应字形），老载荷 / 部署自定义条目可能压根不带该键——两种情况都渲染空槽，槽仍在所以对齐不破。给未知名编一个字形、或回头拿 id 兜底，都是被禁的「编占位」（PRODUCT.md 原则 3）。判定证据在 `e2e/control-row.spec.ts`：同一目录里 `main` 与 `read-only`（**两者都在被删掉的旧 id 映射表里**）并排——前者声明了名 → 有字形，后者缺键 → 空槽；差异只可能来自键在不在。
  - **契约口径**：`icon` 键**恒在**（未声明时为 `null`）；前端窄化只认非空字符串（`api.ts::parseIconName`），其余一律当"后端没说"。⚠ `parseCatalogEntries` 是**白名单投影**——新增契约字段必须在那里登记，否则静默丢掉（#214 落地时实测撞到：后端配好、夹具也带，界面就是不显示）。
  - **本部署可达性**：内置条目恒带 `icon`，所以三条 picker 的**目录条目行**都有字形（首行「默认（未选）」不是目录条目，它一直是无字形的空槽）；空槽只出现在"后端扩展/部署自定义且没声明名"的条目上（夹具用 `longCatalog('mode', 6)` 与显式 `icon: 'sparkles'` 覆盖这两支）。四个调用点都要传 `catalogIcon`（`Composer.tsx` 三处 + `StartTaskInProjectDialog.tsx` 一处），漏传就是静默空槽——`e2e/control-row.spec.ts` 的 AC4 用例逐 picker 断言，正是为了逮这个。
