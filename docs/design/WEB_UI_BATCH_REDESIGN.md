...
- 每行 = 图标槽（20px 固定宽，保证多行对齐）+ 标题（主）+ 描述（次级 2 行内截断）+ 右侧选中标记。
  ...
  - **未知名 / 缺 `icon` 键 → 留空槽，不编字形**：后端可以先行加名（前端还没实现对应字形），老载荷 / 部署自定义条目可能压根不带该键——两种情况都渲染空槽，槽仍在所以对齐不破。给未知名编一个字形、或回头拿 id 兜底，都是被禁的「编占位」（PRODUCT.md 原则 3）。判定证据在 `e2e/control-row.spec.ts`：**同一个 id**（`read-only`）带 `icon` 时槽里有字形、缺键时空槽。
  ...
  - **本部署可达性**：内置条目恒带 `icon`，所以三条 picker 的每一行都有字形；空槽只出现在"后端扩展/部署自定义且没声明名"的条目上（夹具用 `longCatalog('mode', 6)` 与显式 `icon: 'sparkles'` 覆盖这两支）。
