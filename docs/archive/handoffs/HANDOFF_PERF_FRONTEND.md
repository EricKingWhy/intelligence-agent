...
> 本节也**不写「怎么修」**：修复各自归属子票。
>
> 本节共 **3** 条：勘误 1、勘误 2 的过期句子都在本文档内；勘误 3 兼指**本文档之外**的一处错误注释与**本文档内** `:230` 的同源句——两者是**同一个判断、同一次失效**，所以勘误 3 不是「纯文外」条目。
...
| | |
| --- | --- |
| **原句位置** | §9 末段「候选额外优化点」**第 4 项「候选额外优化点」内的第 3 个子句**（本文件 `:243`；该行的列表标号是 `4.`，TurnView 那句是它括号内的第 3 个子句） |
...
**⚠ 本条的「可复现证据」口径（issue AC3 要求逐条写明）**：本条是**状态描述过期**（文档说「未做 / 待证据」，而代码里**已经做了**），**不涉及「谁比谁早」的时间线**，
因此**不适用** `git merge-base --is-ancestor` 那条判据——它的证据是**当前代码**与**ADR 决定**本身（`Conversation.tsx:14`、`:88-99`、ADR-0014 D7），即「现状」而非「先后」。
勘误 1 里那条 `--is-ancestor` 结论是**勘误 1 独有**的论证（两文件恰好同号，容易误读），**不要**以为它顺带证明了本条。
...
### 勘误 3 — 同一断言还有**三处副本**（两处本文档外、一处本文档内；本票只登记，不改代码）
...
- **同一断言的另外两处副本（AC3 要求的「可复现证据」清单，含 `文件:行号`）**：
  - `web/src/lib/projection.test.ts:542-544` —— 就在**守这条契约的那个测试内部**（`it('events 日志数组引用跨事件稳定（O(1) 追加契约，消灭 O(N²) 的前提）')` 的注释写着「events 引用稳定不影响 memo 契约（**无任何消费者把 events 放进依赖数组**）」）。
    ⚠ 这一处最值得警惕：一个**测试文件**在替一个**假前提**背书。
  - `3344e34` 的**提交信息**正文（「…无消费者把 events 放进 memo/useEffect 依赖，已全量核对 App/Conversation/StepDetail/Run Pulse，故不引入 eventsVersion（Simplicity First）」）。
    提交信息**不可改**，此处**只登记**。
  ⇒ 因此 N2 / #271 的修正范围**同时包含** `projection.ts:1149-1154` 与 `projection.test.ts:542-544`；后者改的是**注释**，**不得**因此改动那 7 个引用稳定契约测试的**断言**（`docs/tickets/perf-interaction-smoothness-2026-09-18.md` §0.6 已冻结该测试）。
- **AC3 口径**：本条的「可复现证据」= 上列**三处 `文件:行号`** + 「与 §9 主回执（`:230`）同源」这一事实；它**不是**一条独立的时间线论证，故**不重复** `--is-ancestor`——结论与勘误 1 相同（同一次失效：`f97f322` 晚于 `a78c322`）。
