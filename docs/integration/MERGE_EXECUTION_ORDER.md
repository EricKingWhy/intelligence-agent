...
> 与现行 `AGENTS.md` §14.4 冲突——**授权一律以 §14.4 为准**：集成与集成后的
> `push origin main` 是**常设授权**（当前主开发执行）；feature / 短分支上的 push、
> GitHub PR Merge、cherry-pick、revert、冲突解决后的 `git add`、删分支/worktree
> 仍需**单独批准**。
>
