...
> ## ⚠️ 历史快照，勿照此执行（2026-09-16 标注）
>
> **本单描述的那次合并早已执行完毕**：它引用的三个 sha —— 源 tip `697a085`、
> 目标 `ebb2d68`、merge-base `c00f742` —— **现在都是 `main` 的祖先**（实测
> `git merge-base --is-ancestor <sha> main` 全部成立）。本单里的 sha、`ahead/behind`、
> 测试数、文件数全是 **2026-09-10 18:40** 的值，**按它施工会去合一个根本不存在的状态**。
>
> **授权口径也已过期**（见下方就地标注）：本单第 4 行写「merge 与 push 均需用户明确批准」，
> 与现行 `AGENTS.md` §14.4 冲突——**授权一律以 §14.4 为准**。
> ⚠️ **2026-09-26 再次变更**：`main` 已开服务端分支保护（必需检查 `gate0`、`enforce_admins` 为真），
> `push origin main` **不再是常设授权**——它被服务端直接拒绝（`GH006`）。集成改走
> 「推集成分支 → 开 PR → `gate0` 绿 → 合并 PR」，而这两步各自**仍需单独批准**；
> GitHub PR Merge、cherry-pick、revert、冲突解决后的 `git add`、删分支/worktree 同样需单独批准。
> 故下方正文里那句 `git push origin main`（§196）**现在物理上不可执行**。
>
> **保留它的理由**：它是「跨两个独立仓库执行合并」的操作范例——§1.1 的
> 「两个仓库不共享对象库」实测事实与 §2 的取值纪律仍然成立，可作模板参考；
> 但**每次执行都必须重新实测**（`git fetch` / `diff` / `merge-base`），
> 不得复用本单的任何数字。当前生效的集成流程见 `AGENTS.md` §13.4 与 §14.6/§14.9。

> **给集成 AI / 执行合并的授权角色。**
> 依据 `AGENTS.md` §14（Git Workflow / Merge Safety）。~~**merge 与 push 均需用户明确批准。**~~
> → **该句已被 §14.4 常设授权取代（2026-09-16），以 §14.4 为准，见本页顶部横幅。**
>
