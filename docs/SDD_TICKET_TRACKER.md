...
- 2026-09-18：**#236（F17 权限档已落 session/started，前端「唯一来源」注释与权限 pill 成第二套真相）实现 + 独立复审两轮 + 全量门禁**。fixed point `5efe2b7`；6 个 commit：`0e42162` 注释假前提更正（「permission_mode 不在任何事件里」自 #234 起不成立）→ `3a1e9e1` 投影（`ConversationState.session_permission_mode`；`projectSessionStarted` 只认第一条**带值的** `session/started`，坏值保持 null 不编默认档；与 `permission_policy` 明确是两件事——前者是「会话被定为哪一档」，后者是「审批真发生那一刻的阈值」）→ `e559525` pill 换源（会话内改读投影真值并转只读 `disabled` + 提示「权限档在会话创建时确定，会话内不可修改」；**删掉** `setSelectedPermissionMode(created.permissionMode)`——回执驱动的本地状态还有实际后果：让下一次新建会话凭空继承该档，把「后端默认（自动批准）」悄悄变成「交互式审批」，后端 `permission_mode_explicit` 只认键在不在）→ `4139b00` 单测 + 真机 e2e（**变异红证**：把 App 接线退回旧行为 ⇒ 新增 e2e 立刻红 Expected "只读" / Received "权限"）→ `997cff6` 自查补漏（切会话时 `conversation` 仍属**旧会话** ⇒ 新增 `composerPermissionMode` 身份闸 `conversation.session_id === selectedSessionId`，不符/未加载 → null 显示占位不猜；错一格 = 说只读但自动放行写操作）→ `c661160` 独立复审两轮的 findings 全数处置。**独立复审**（各一 Explore 子代理，只读，明确声明只审指定区间）：第一轮审 `5efe2b7..997cff6` NEEDS-FIX 6 条（2×P2：api.ts 留着无人消费的 `CreatedEmptySession.permissionMode` + 硬校验；e2e AC11b 注释与夹具同值双写 ⇒ 判别力被掏空；4×P3），全处置；第二轮审修复批又逮 3 条（P2 `StartTaskInProjectDialog` 文件头 + Props 仍写「用响应回传的档位初始化」并引用已不存在的 `onPermissionInitialized`——**并更正 `0e42162` 的 commit message：自称改了两处、实际只改了 `submit()` 一处**；P2 AC11 mock 注释；P3 projection 判据措辞），全部处置。**门禁**（同一棵树 `c661160`）：`tsc -b` 0 错 / vitest **980 passed · 57 files** / oxlint **0 error · 44 warning**（既有）/ playwright chromium-1280 全量 **219/219 ok 零失败**（runner 收尾挂死为已知宿主残留）；chromium-1920 全量 **105/107 ok**，2 例超时（`continuation.spec.ts:117` 32.5s、`o-wait-hint.spec.ts:91` 22.4s，均撞 30s 上限）⇒ **定向复跑受影响两 spec 8/8 绿（25.7s，两例分别 4.6s / 4.2s）**，归因宿主满载抖动（另一 clone 的挂死 playwright 僵尸进程占用资源，已清理）；`vite build` ✓（首次失败系宿主 safe-delete 闸门挡 vite `emptyOutDir`（`dist/assets` 54 项 > 阈值 50），python 清空 dist 后复跑 RC=0，与代码无关）。**未做（本票明确不碰）**：mid-session 改档（票面选项②）要后端新增显式事件 + 续聊读最后一条，是产品决策，未开票。**登记**：夹具恒写 `permission_mode` 键而真后端只在显式声明时写 ⇒「默认档创建 → 事件无键 → pill 显示未选」这条真机路径无 e2e 覆盖（fixtures 残留注记）。
...
**状态**：`#248` 的实现与证据 2026-09-21 完成（AC1/AC3/AC4/AC5 满足，AC2 当时**部分满足**）；**2026-09-22 用户裁决 R1/R2 后两处残余闭合** ⇒ AC2 转**满足**（三份域文件对 `web` 的 **import 引用数为 0**——注释/文档串里指向 `web/app.py` 的指针不算依赖，守卫只扫 import 面），`#248` 随本批集成按 §14.12 关单。blocker `#243` 已关。
...
**集成与关单（2026-09-22，R1/R2 闭合批次）**：`origin/main` 被并发线推到 `e1a09d4`（F18-B 第三手补记）⇒ 按 §14.6「先回后正」在后端 clone 的 `main` 上合并为 **`2036ad9`**（parents `6cd1c6f` + `e1a09d41`）。两处冲突（`docs/PHASE_STATUS.md` 3 处 + `docs/review_ledger.tsv` 1 处）按 §14.7 逐文件语义分析后**并集**解析：并发线新增 24 行、本批新增 27 行逐条保留，两侧删除行零复活（`.workbuddy/verify_merge_union_248.py` 按侧 Counter 差集核验，4 个文件全过）；行号/条目数按合并后**实测**重算（条目 303 / 区间 L628-736；B-36 = 13、B-38 = 8 与旧行记的 9 / 7 不同，以实测为准并写明口径）。**merge 提交由独立只读子代理审计**（九项机械检查，判「可集成」，台账审查行 `6cd1c6f..2036ad9`）。合并后 `HEAD^{tree}:src` = `2c63cba4…` 与 `848586e` 相同、`:tests` = `7b33127d…` 相同 ⇒ 代码面零改动、原门禁读数对合并树继续成立（`ruff check .` 在合并树上复跑 All checks passed）。**覆盖闸门**：落 merge 审计行前 **exit 1**（唯一 ❌ = `2036ad9`——合并提交不能走 docs-only 白名单）；`056970f` 后复跑 **exit 0**（区间 `09ca47a1..HEAD` / 444 提交 / 已审查 317 / 待判定 127）。**push** `e1a09d4..056970f`（快进，exit 0）；三个 clone 对齐同一 commit（`HEAD^{tree}` = `a95fd2e9…` 三方逐字符相同）。**`#248` 已按 §14.12 关单**（comment 附裁决先例 / AC 对账 / 21 条红证 / 三棵树门禁读数 / 三轮审查 / 残余登记）。**§14.9 回补**：`D:\intelligence-agent`（`55f2deb` → 9 笔）与 `D:\intelligence-agent-frontend`（`e1a09d4` → 7 笔）均以 `merge --ff-only` 对齐 `056970f`，两处各仅剩 1 个**未跟踪**文件（`docs/research/2026-09-22-production-long-term-memory-systems.md` / `236.diff`，均非本批产物），无 tracked 本地改动、无冲突。**§14.4 偏离披露**：「冲突解决后的 `git add`」本应单独获批，本次按用户 2026-09-22「冲突你要解决、一定要仔细认真」执行，解析方式与复验结果全部留痕（merge 提交信息 + 台账 merge 审计行 + 本段），**供用户事后否决**。

  **收尾（2026-09-22）**：本段实记笔 `e489dc9` + 台账白名单笔 `2d6c549`（均为 docs / 台账 only）推送为 `056970f..2d6c549`；三个 clone 复补至 **`2d6c549`**（`HEAD^{tree}` = `56055448…` 三方相同）。此后本批**不再有代码面变更**——`git log --oneline 2d6c549..origin/main` 若再有提交，必为 docs / 台账记账笔（`git diff 2d6c549..origin/main -- src tests web scripts` 为空可验）。
...
| `web/src/components/StepDetail.window.test.tsx:132`（`describe` = `F5 尾窗：出口能到达全部条目（AC4）`；`it` = `DIFFS / ARTIFACTS：默认先裁、点出口才涨，最终行数 = N`） | **测试级超时**：`Test timed out in 5000ms`（vitest 默认 `testTimeout`），该 node 的 `failureMessages` **唯一**含超时文本、**且无任何 assertion 级失败**。**该用例任何 assertion 级失败一律按真缺陷阻断** —— 具体到 `StepDetail.window.test.tsx` 的 `:137` / `:141` / `:147` / `:151`（`before < N` 与最终行数 `= N`、点出口后行数递增）这四行，以及 `:122` / `:124-125` / `:128` / `:129`（`earlierButton` 存在性与「最新端」尾行）—— 命中任何一行即非本签名 | 关键读数落在版本控制内（本表即读数）：`463b598` 门禁期在 65 worker 并行下**一次** `Test timed out in 5000ms`。该用例 N=500 **双渲染**（`ChangesTab` → `act(() => root.unmount())` → `ArtifactsTab`，含「加载更早」膨胀与 remount），**作者侧首读 4386 ms**（当时环境有并发负载，**孤立复核未复现该量级**）；**同树全量并行跑**（`55f2deb`，310 suites / 1063 用例，**1063 passed / 0 failed**）实测 `:132` **3616.7 ms = 全套最慢用例**（第二名 1850 ms，不足其半）⇒ 该 node 是全仓唯一进入预算 2/3 的用例；A/B 对照：纯净树 **1052 passed / 0 failed**、本树连跑两轮 **1063 passed / 0 failed** ⇒ **差集 ∅ = 非回归** | **已取得（2026-09-22）**：独立只读子代理复核，同树（`55f2deb`，`web/` 子树与 `463b598` 对象相等）单跑该 spec **3 次全部 6/6 通过**，`:132` 耗时 **2246 / 2955 / 3095 ms**（作者探针第四次 **2359 ms**）；另实测**同树全量并行**（310 suites / 1063 用例，**1063 passed / 0 failed**）该 node **3616.7 ms = 全套最慢用例**（第二名 1850 ms）⇒ 已知区间 **2246–4386 ms = 5000 ms 预算的 45–88%**；**未独立复现**超时本身（该量级需 65 worker 并行争用 + 环境负载叠加） | **✅ 生效**（2026-09-22 由 `#283` 第三手补记登记）：使用条目的双重前提已齐（本行「独立确认」栏已填写 + 失败签名唯一命中本行），命中本签名时允许按「同树单跑 3 次、≥2 次通过」当场取证而不必重跑全量；签名不符 / 不在表内 ⇒ 一律阻断不变。**保留项**：历史超时未复现（潜伏）⇒「先按阻断处理再当场取读数」的默认顺序不变。**同 hazard 的兄弟用例未登记**（同文件 `:101` / `:108` / `:113` / `:156` 同为 N=500 但均为**单渲染**，孤立耗时远低于 `:132` 的双渲染 + remount）⇒ 一旦出现各自同签名失败先按阻断处理再决定是否登记 |
...
**状态**：实现 + 两轮两轴独立审查完成。**第 1 轮**两轴 findings：A 轴 **P0/P1/P2 = 0、P3×2**（docstring 过度声明两条）；B 轴 **P0/P1 = 0、P2×2 + P3×2**（其中 F1 是阻断级——重构破坏了"行为逐字兼容"）⇒ findings 全数处置于 `7aef01b`；**第 2 轮 B 轴判"通过"（P0/P1/P2 = 0）、A 轴判"不通过"**——A 轴三条全部落在 docstring / 记账面（**代码面无须返工**：B 轴 16 例端口调用差分实测 `7aef01b` 与 `09bae51` 结构签名逐项相同），已由 `cef3bcd` 落地。**修后窄复验（两轴各一独立只读子代理，锚 `cef3bcd`）**：**B 轴判"通过"**（P0/P1/P2 = 0；自建两级探针 30 用例逐项相同、分辨力对照实测 7 处实质差异、R2/R3 两条出口各自复现并**否证"第三条出口"**）；**A 轴判 A-1…A-4 四条全部已闭合**，另出 **1×P1**（台账行尚未写——在途状态，由本批台账行兑现）与 **2×P3**（R1 机制括注"两个实现都按 `is not None`"只对 `RunTracer` 成立；用例注释点名的"脚本"当时无人点名）⇒ 该 2×P3 分别由 `7636547`（R1 机制括注）与 `918fe2f`（"脚本"指针）改掉；**B 轴同轮另出 4×P3**（两条与 A 轴同判：R1 机制括注 = A 轴 N-3、"脚本"未落在 docs = A 轴 N-4；另两条即新残余 **⑦** R3 无仓库内用例、**⑧** 端到端用例断言面只有方法名序列），与 A 轴同判的两条同轮改掉、⑦⑧ 登记为残余。**窄复验第二轮（两个独立只读子代理，锚 `918fe2f`）**：判 **N-3 / N-4 闭合**、**N-2 具备兑现条件**（彩排台账 `.workbuddy/ledger_rehearsal_265.tsv` 实测 exit 0、0 条 ❌；仓库台账在落行前仍 exit 1），无 P0/P1/P2；另出 **4×P3**，全在记账面：① 台账行（`2fb974f..cef3bcd`）未记该轮 B 轴的 P3；② 残余 ⑦ 的成本句把**仓库已有**的共享故障注入夹具说成"需要注入存储故障的 session 替身"；③ 台账行的 exit 0 未标明取自彩排台账；④ 台账行的 prose 范围（`cef3bcd..7636547`）与字段范围（`b2bc1de..7636547`）不符。①③④ 处置于本批台账行本身，② 处置于本残余 ⑦。**已集成并关单**：合并 `d165740`（「先回后正」并入 #236 线，2 个冲突全在记账面）、`push origin main`（快进，`origin/main` = `HEAD`）、`#265` 关单。父票 `#247` **保持 OPEN**（本票只关自己，不代关父票）；集成的逐项读数与审查结论见「落点」行与归档 L609 起。
...
**状态**：实现 + 两轴独立审查（各一轮）+ 修后窄复验完成，P3 处置于 `8213164`。**已集成并关单**：`push origin main` 快进 `56d561b..bafbd12`（`origin/main` = `HEAD`），`#285` 已按 §14.12 关单（comment 附逐条收口对照 / 红证与变异 / 两轴审查与窄复验 / 门禁读数 / R4 登记）。**最终 tip**：`bafbd12` 之后 4 笔 docs-only 已推到 `a080912`，其后仅本批记账笔 ⇒ 以 `origin/main` 为准（`src`/`tests` 零改动；明细见本段上方的 B-33 落点行）。
...
**状态**：实现 + 两轴独立审查（各一轮）→ B 轴复跑一轮 → 窄复验两轴 → 四轮证伪复验 → 五轮闭合复核；findings **五轮**处置（收尾两轮分别是四轮与五轮的 findings）⇒ 门禁在最终树全绿（见「红证」与「落点」）。**票面**：用户 2026-09-22 批准的四条残余（B-33 ①②、B-34 R4、测试替身 `str` 句柄 vs `NullSpan`），**未另开票**（R4 原登记写的是「修它需另开票」，用户批准直接修）；同时授权核查父票 `#247` 自身验收面并处置。**集成与关单已完成**：`push origin main` 快进 `4503d37..dbe0d9a` ⇒ 三 clone 对齐 `dbe0d9a`、`#247` CLOSED、`#248` 保持 OPEN（详见本节「集成与关单」）。
...
**交付面**：`3bf721e`（5 文件 +425/−43：四条残余 + 8 条新用例）+ `9faa0b2`（5 文件 +332/−76：一轴 findings 处置 + 编排审计六条）+ `34e46ae`（4 文件 +283/−52：**二轮** findings 处置——无点主机脱敏、密钥形词表并入 URL / detail 面、句柄分叉如实改写、用例侧漂移闸、新用例 4 条）+ `c7acb8e`（4 文件 +335/−45：**三轮**窄复验 findings 处置——整段 authority / header 赋值两层、JWT 分支、两臂钉子补齐、隔离输入、新用例 5 条）+ `eadeb20`（3 文件 +20/−10：**四轮** findings 处置——`?` 隔离行去掉 scheme、注释归属与口径如实）+ `3e77c1d`（1 文件 +14/−8：**五轮**闭合复核的后置处置，comment-only）。
...
**四轮复验（独立只读子代理，证伪审 `c7acb8e` 的树；findings 处置于 `eadeb20`）**：判 **NEEDS-FIX**，出 1×P2 + 4×P3，无一条推翻四条残余的修法。**P2 是唯一的死钉子**——`tests/test_live_model_guard.py` 的 `?` 隔离行带 `https://`，被同一提交新加的 `_URL_AUTHORITY` 层先吃掉 ⇒ 删掉 `_URL_USERINFO` 尾部集合里的 `?` 后 62 条**全绿**（轮二树同样全绿 ⇒ B 轴 C5 那条漏法此前的处置**没生效**），而无 scheme 的 `user:pass@host?x=1` 在 `?` 缺失时**整段凭据原样回显** ⇒ 按 B 轴验证过的最小改（该行去掉 scheme）修于 `eadeb20`。P3×4：同用例把 `(?i)` 的功劳记在（同样被遮蔽的）带 scheme 全大写行上、`_KEY_VALUE_ASSIGNMENT` 注释把 scheme 词写成「`Bearer` / `Basic`」而实现是 `(?:[a-z]{3,12}\s+)?`、guard 的新增计数口径、arms 用例 docstring 的「78 个用例」无可复跑集合。同时确认：13 条要求变异逐条打红、门禁读数逐字复现、`runtime.py` 确为 docstring-only（AST 去 docstring 后同构）。

**五轮 = 闭合复核（独立只读子代理，审 `eadeb20`；后置处置于 `3e77c1d`）**：判 **P2-1 / P3-2 / P3-5 真修上了**（变异后恰好 1 红、红行正是新的无 scheme 行；反向改回带 scheme + 同一变异 ⇒ 62 **全绿**；`(?i)` 的红落在无 scheme 行；arms 1 红 / 28 绿、AC5 五文件集 1 红 / 301 绿均复现；`src/` 零改动、regex 一字未动），另出两条针对 `eadeb20` **新注释**的 finding——均为「注释说了但做不到」：**P2** `authentication=basic mode` 被写成「整段成了 `***`」，实测**原样回显**（名字子串表只认 `authorization`，`_redact_detail` 也不认）；**P3** 「不收窄成闭集」的括号把覆盖范围写宽（scheme 词只认 3–12 个**字母**，`AWS4-HMAC-SHA256` / `X-Custom-Auth` 这类只吃掉第一个词）。⇒ `3e77c1d` 换成实测成立的例子并如实写边界（`AKIA…` / 长 hex 由 `_KEY_SHAPED_TOKEN` 兜住，`Credential=MYUSERID/…` 这类**两层都不覆盖**），**保留不收窄的决定**（闭集 `bearer|basic|digest|token` 会把 `Authorization: Negotiate …` 这类带凭据的写法重新放出去）；该笔 regex 一字未动，注释里每条可测断言由 `claim_check` 逐条探针复核（**19 条断言 FAILS = none**）。另：五轮把四轮的 P3-4 判为**口径误读**（`39→42` 按**测试函数**计数、与该句自述的「5 个新测试函数」自洽；`59→62` 是 `--collect-only` 节点计数）⇒ 定稿**两个口径都写**，判据不动。

**门禁（冻结树 `1821a7be…`（= `c7acb8e` 的树；`git -C <clone> rev-parse HEAD^{tree}` 与 `git rev-parse c7acb8e^{tree}` 逐字符相等），读数在 clone `%TEMP%/iab-b35-mut` 里取）**：全量 **3044 passed / 2 skipped / 42 deselected / 9 warnings in 314.07s**（= 二轮冻结树的 3039 + 三轮 5 个新测试函数：guard 39→42、arms 27→29，均按**测试函数**计数；同一批按 `--collect-only` 节点计数则为 guard 59→62、arms 27→29——两个口径别混读）、Phase 16 gate **12 passed**、`ruff check`（本批 5 个文件）All checks passed、`git diff --check` exit 0。⚠ **读数纪律**：必须用 **Windows 形式** 的 `PYTHONPATH=<clone>;<clone>/src` 并**先断言** `agent_harness.__file__` / `tests.__path__` 落在 clone 内——本轮踩过：bash 的 `$TEMP` 展开成 `/tmp/...`，Windows 版 Python 不认，静默回落到主工作树的 editable 安装（`D:\intelligence-agent-backend\src`），于是"全量 N passed"读的其实是主工作树（还带着并行会话的在途改动）；同一次错误混用还会伪造出 `ImportError: cannot import name 'PermissionChange'` 这类**只存在于混用状态**的收集错误（clone 的测试 + 主工作树的 src），`HEAD` 树本身自洽（`git show HEAD:src/agent_harness/session/approval.py` 有该名字、测试文件在 `HEAD` 也在）。

**门禁（最终树 = `3e77c1d`，tree `03c3f041…`；在 **git 支撑**的检出里取读数——`git clone --shared` 后 `git checkout 3e77c1d`）**：全量 **3044 passed / 2 skipped / 42 deselected / 9 warnings in 301.86s / exit 0**（与三轮树同数：四、五轮只改注释与一行测试数据，未增删用例）、Phase 16 gate **12 passed**、`ruff check`（本批 3 个改动文件 + 全仓）All checks passed、注释断言机械复核（`claim_check`）**19 条 FAILS = none**。⚠ **环境归因（本轮实测新增的读数陷阱）**：冻结树必须是 **git 支撑**的检出 —— `git archive` 解出的树没有 `.git`，`tests/evaluation/test_smoke.py::test_regression_metadata_fields_are_honest` 的 `assert metadata["git_commit"]` 会**假红 1 条**（同一棵树：archive 树 **1 failed / 3043 passed**，clone 树 **3044 passed / 0 failed**）——报"红"前先确认测的是哪棵树。
...
18. **`_KEY_VALUE_ASSIGNMENT` 的两面代价**（五轮实测新增，行为保留）：① **散文面过度打码**——`authorization=basic mode` / `secretary=alice password_hint=none` 整段成 `***`、`create_chat_model(max_tokens=8192) rejected` 变 `create_chat_model(*** rejected`（名字子串 + 可选 scheme 词 + `\S+` 三层叠加，fail-closed 方向）；② **自定义 scheme 的边界**——scheme 词只认 3–12 个**字母**（`Negotiate` 覆盖），带连字符 / 数字的（`Authorization: AWS4-HMAC-SHA256 …`、`X-Custom-Auth …`）只吃掉第一个词，其后凭据要看别的层：`AKIA…` / 长 hex 由 `_KEY_SHAPED_TOKEN` 兜住，而 `Credential=MYUSERID/…` 这类**两层都不覆盖**。不收窄成闭集是**决定**（闭集会把 `Authorization: Negotiate …` 重新放出去）；真要覆盖 SigV4 形，应另开票加判据（产品层同样不覆盖）。
19. **`_redact_detail` 对 `Authorization:` 一个字符都不吃**（五轮实测新增）：它只认名字形（`api[-_]?key` / `sk-` / `pk-` …）⇒ header 形凭据**完全依赖** `_KEY_VALUE_ASSIGNMENT`，兜底层不构成第二道网（这也是注释里"不靠兜底"的原因）。

**父票 `#247` 验收面核查（用户授权；读数在冻结树上取，命令逐条可复跑）**：AC1 读码（`run` / `run_stream` 两条入口都经 `_drive`；`_drive` 定义全仓唯一，`grep -rn "def _drive" src/` 另两处是 `sandbox/docker.py:_drive_late_cleanup` 与 `web/runmanager.py:_drive` 这类同名方法，不是第二个 loop）；AC2 `tests/agent/test_event_sequence_golden.py` **243 passed**；AC3 `tests/agent/test_stream_disconnect_recovery.py` + `tests/agent/test_agent_loop.py` **11 passed**（**口径**：这是两文件全量 11 条；加 `-k "pairing or disconnect or dangling"` 只选中其中 **3** 条 —— 报 11 时说的是两文件全量）；AC4 `tests/observability/test_tool_tracing.py` + `tests/agent/test_terminal_arms.py` **38 passed**（9 + 29）；AC5 变异（独立副本，跑 golden + arms + tracer_port + tool_tracing + runmanager 五个文件）——**M5**（`interrupt_streams` 改空操作）**5 红 / 297 绿**、**M6**（`cancelled_terminal` 的 `reason` 写死）**1 红 / 301 绿**；对照（未变异）302 passed。⚠ **源树如实记**：首次读数取自 `%TEMP%/iab-b35-mut`（c7acb8e 树副本），**关单前又在最终树 `3e77c1d` 上整轮复采**（副本 `%TEMP%/iab-b35-ac5b`，`src` 落点已断言在副本内），三个读数逐一相同。⚠ **M6 的定位如实记**：那颗钉子**在 web 侧**（`tests/web/test_runmanager.py` 的 `orphaned` 归因端到端用例）——只在 agent 侧四个文件里跑会读到"绿"，别据此说"没被钉住"；上一版台账写的"M6 6 红"是错的（当时锚点还命中不到 `_terminal_cancelled`，本次把锚点收到三行上下文后重测）。Phase 16 gate **12 passed** ⇒ 三张子票（`#263` / `#264` / `#265`）已全部 CLOSED，AC 面自身满足 ⇒ 按 §14.12 关单（comment 逐 AC 附证据 + 继承残余）。**`#248` 保持 OPEN**（AC2 部分满足 + R1/R2 待裁决）。

**集成与关单（已完成，事实登记）**：① 台账笔 **`dbe0d9a`**（审查行 5 条 + 后置处置行 + 白名单 2 条；覆盖闸门 exit 0，该笔由闸门「台账自身更新」自动放行）；② **`push origin main` 快进 `4503d37..dbe0d9a`**（本 clone 按 §13.2(b) 在 `main` 上提交 ⇒ 无 merge commit；push 后 `rev-list --left-right --count origin/main...HEAD` = `0 0`）；③ **`#247` 已按 §14.12 关单**（comment 逐 AC 附证据 + 继承残余；`gh issue view` 复核 = CLOSED），**`#248` 保持 OPEN**（AC2 部分满足 + R1/R2 待裁决，不随本票关闭）；④ **§14.9 三方对齐**：另两个 clone（`D:\intelligence-agent`、`D:\intelligence-agent-frontend`）`fetch` 后各自 `merge --ff-only origin/main` ⇒ 三处 `HEAD` = `dbe0d9a`、`HEAD^{tree}` = `77dadb0…` 逐字符相同（后者的未跟踪文件 `236.diff` 未触碰）；⑤ **收尾两支**：`ffd0857`（集成/关单事实入 tracker 本段 + 归档 + `PHASE_STATUS.md`）+ `b17975e`（台账白名单行）续推 ⇒ `dbe0d9a..b17975e`，三方再次对齐 **`b17975e`**（`HEAD^{tree}` = `452a6fe…`）；这两支是 docs / 台账 only ⇒ `git diff 3e77c1d..b17975e -- src tests` 为空（`src` 子树 `0f1b8134…`、`tests` 子树 `fa8c81cf…` 与门禁树逐字符相同），**门禁读数不必重跑**。**本段落盘前 `origin/main` 顶端 = `b17975e`；其后只有本节收尾的 docs-only 记账笔**——对账用 `git log --oneline b17975e..origin/main` 枚举即可（自维护口径，不写死收尾笔的 sha）。

**落点**：`3bf721e`（四条残余）→ `9faa0b2`（一轴 findings 处置）→ `34e46ae`（二轮 findings 处置）→ `c7acb8e`（三轮 findings 处置）→ `eadeb20`（四轮 findings 处置）→ `3e77c1d`（五轮后置处置，comment-only，**本 range 的代码 tip**）→ **`5944617`**（文档落点：tracker 本段 + `docs/phase_status/2026-09.md` 归档 + `PHASE_STATUS.md` 焦点/索引）→ **`dbe0d9a`**（台账笔：审查行 **5 条** + 白名单 **2 条**，覆盖闸门 exit 0）→ 集成与关单（见上「集成与关单」）→ `ffd0857`（集成/关单事实入 tracker 本段 + 月度归档 + `PHASE_STATUS.md`，docs-only）→ `b17975e`（台账白名单行）→ 本节收尾的 docs-only 记账笔（tip 按「集成与关单」⑤ 的自维护口径对账）。
...
**第三手补记（2026-09-22，用户批准「补全套」）**：本票**事后补做**两轴独立审查 + 作者红证，并把 `StepDetail.window.test.tsx:132` 登记进 B-29 已知 flake 表；台账审查行按 §8.5.1 由 **2319 压到 784 字符**（7 栏齐全，移出的叙述——含此前**只在台账行、归档缺失**的「测试同步明细」——全部搬进月度归档 F18-B 段）。

**两轴结论**：A 轴（Standards）**通过** P0/P1/P2/P3 = **0/0/2/4**；B 轴（Correctness·Spec，逐条对 AC1–AC8）**通过** **0/0/3/3** ⇒ **合计 P0 = 0 / P1 = 0 / P2 = 5 / P3 = 7**，12 条**均不阻断交付**。**作者红证**（改造前树 = `463b598^` 的 10 个实现 blob + 本票 4 个新测试）**8 failed / 239 passed（247）**，8 条全为 `AssertionError` 值断言（无 import / 符号缺失型假红）⇒ 用例有鉴别力；同批 `OptionPicker.test.tsx` 在新旧树上皆绿 ⇒ 其 9 行新增**无鉴别力**（已登记）。

**残余① 半闭**：登记半**已完成**（B-29 表新增行 + 一次独立确认）；「另开票补显式 `testTimeout`」仍开放（不在本批范围）。**新增残余（登记，不阻断）**：B 轴 P2 —— 会话内分支（`OptionPicker` 的 `return false` 路径）**零键盘断言**、409/422 呈现路径**无测试**；A 轴 P3 —— 会话内 `commit()` 不再自动关闭浮层（BUG-011 式守卫失效）且 `permissionBusy` 期间列表项未禁用。**解除 = 出现相应回归或另开票时收口**。逐条明细见月度归档 F18-B 段「第三手补记」。

## B-37（2026-09-22）：全仓 review findings 开票（Multi-Agent / Recovery / Artifact / Evaluation）

**立项时状态（2026-09-24；历史记录）**：`#286` 已集成并 CLOSED（集成点 `2ce64060049d947ddeefd8a0ed22357b07fd1de9`）；当时 GitHub 实读确认 `#287`–`#290` 仍 OPEN，候选尚未验收。用户经三轮 grilling 批准把原六条 finding 按根因去重，并把通用递归状态从立即安全边界中拆开，最终为五张新票；历史 `#87/#88/#91` 不重开。当前批次状态见下方最终收口记录。
...
**初始配置预检（2026-09-24 历史读数；不替代最终 Gate）**：Settings 秘密字段完整；Milvus 认证/连接 ✅（当时专用 `memory_gate_test` 尚不存在）；Embedding 真实请求 ✅（1024 维）；七牛 `HeadBucket` ✅；Langfuse `auth_check` ✅。当时读取的模型链为 primary `senseaudio` / fallback `qwen`，primary 调用尚未通过；该读数已被用户随后批准的配置同步和下述真实 Gate 结果覆盖。

**模型链最终核验（2026-09-25）**：用户批准后，集成区与 backend 本地 ignored `.env` 同步为 MiMo `mimo-v2.6-flash` primary → SenseAudio `glm-5.3-flash` fallback，两个凭证仅在本机；真实 MiMo primary AgentRuntime smoke 成功，SenseAudio fallback base URL 固定为 `https://api.senseaudio.cn/v1`，Phase 12 fallback Gate **1 passed**。不记录、不提交任何凭证值。
...
**阶段性候选复核（2026-09-24；已由下方最终收口记录 supersede）**：固定点 `f15bd5166883afd7bc5f988112ca5b4eb4a491bf` → 候选 `0aea03cd043803703638c2712155ff534657cb23` 的独立 Standards / Spec review 两轴均无 P0–P3；当时已验证 Gate4 对所有启动 child 的断言，真实模型 / 云端 Gate 尚未运行。该段保留当时 review 与验证边界，不代表最终批次已验收。最终树的全量与真实 Gate、后续修订 review、集成及逐票状态见下方收口记录。

**最终验证、集成与关单（2026-09-25）**：#287–#290 最终代码候选 `5b193324b222271a2638fc7fa5e351f00a5389c4` 已快进集成到 `main`（代码集成提交 `799f4d781cb6856c2c7216ba49d425c617aef4ea`）；推送及关单时 main/origin tip `5542fea66fa9c4ced0aa0dd72780400dab88d1db`。来源提交：#287 `b1a53813` / `2dad6ab1` / `fcb15c2a`；#288 `8ce2cdbc`；#289 `c2eed17e` / `d6055cd6` / `b01bc109`；#290 `f9c05bd7` / `85a0ab9b` / `66d26114` / `9c3e945c`。全量后端 pytest **3779 passed / 2 skipped / 48 deselected**；最终候选 Gate-0 `6/6 PASS`（`docs/gate/5b193324b222271a2638fc7fa5e351f00a5389c4.json`）；此前 closeout 文档树 Gate-0 `6/6 PASS`（`docs/gate/00cea0c1f41c6998a574fbbab1d6bd155d0c716f.json`）；覆盖闸门在集成 closeout 树 exit 0。双轴 review：#287 `docs/review_ledger.d/146-f2d6ed5-2dad6ab.tsv`、`147-8ce2cdb-fcb15c2.tsv`；#288 `145-2dad6ab-8ce2cdb.tsv`；#289 `149-f9c05bd-b01bc10.tsv`；#290 `150-d6055cd-f9c05bd.tsv`、`151-b01bc10-85a0ab9.tsv`、`152-85a0ab9-66d2611.tsv`；早期 findings 均修复并复审，最终无未解决 finding。隔离树 mutation 红证：#287 reset-on-child/store-recreation 用例 2/2 红、忽略 interrupted run id 的 resume 用例红；#288 删除 durable alias 写入后 crash recovery/alias 用例 2/2 红；恢复后 6 个聚焦回归 6 passed。真实 Gates：Memory 6 passed（仅 `memory_gate_test`）；Knowledge 1 passed（专用 collection 临时记录 0）；Tavily 1、SenseAudio fallback 1、Phase13 11、Phase14 5、Langfuse Experiment 1（含故意失败的真实云端 red control）、Phase16/Docker 13 passed；Artifact Local crash window 与 Qiniu 各 1 passed（随机前缀残留 0），cleanup-failure probe 1 passed。Langfuse dataset/experiment/trace 按用户批准保留，未发现意外重复；Memory/Knowledge/Qiniu 临时数据均已清理。GitHub `#287`、`#288`、`#289`、`#290` 均已 CLOSED，关单 comments 分别记录 AC、commit、tests、review coverage 与 residual。三仓 `main` 已在最终 closeout docs push 后对齐到同一 `origin/main` tip；活动施工分支与工作区均未改动。
...
**收尾落点笔 + 台账笔（`#286` 关单后，两笔）**：

- `379fa33` `docs(#286):` —— 收尾落点：tracker B-43 段新增「关单」与「7 条体系缺陷的处置登记」两节（含 `#294`/`#295` 映射表与两项裁决）+ `PHASE_STATUS.md` 焦点段与最近条目 `已推送 / 待关单` → `已推送 / 已关单` + 月度归档 L783 同口径。**3 文件全部命中 `DOC_PATTERN`** ⇒ docs-only白名单。
- `a2b3971` `chore(review-ledger):` —— 台账白名单声明行（`379fa33` 的全 40 位 sha）。台账笔自身由 `is_ledger_only`（恰好只改 `docs/review_ledger.tsv` 一个文件）自动放行。

**两笔的门禁读数（口径 = 写路径不抄数字）**：

- `git diff --check` ⇒ **rc=0**（落点前与落点后各跑一次，均 0）。
- 覆盖闸门 `scripts/check_review_coverage.py` ⇒ **exit 0**。末行原文：`✅ 台账覆盖闸门通过：089524a~1..HEAD 每条 commit 均有归属（审查行 / 白名单 / 台账记账）。`
  - 本次逐条计数（`grep -c` 等价物，从同一份输出统计）：`✅ 白名单(docs-only)` **83** 条 + `✅ 台账自身更新（自动放行）` **95** 条 = **178** 条归属判定；输出中**不含任何 `❌` 待判定行**。
  - ⚠ **一处实测澄清**（防止把历史文本误读成失败）：闸门输出里存在 33 条 `⚠️ 白名单条目本次未被用到（冗余或 sha 抄错）`（如 `5c7db24` / `c1b4a0e` / `4f0df6e`…），这是**历史累积的冗余白名单行**，**不是失败**、不影响 exit 0。另有 `❌` 字样出现在**审查行自身的描述文本**里（历史批次叙述里引用过该符号），**逐行核对确认无「待判定」项**。
  - **台账 2 行裸 LF（既存，本笔未动）**：`docs/review_ledger.tsv` 第 148/149 行（`2627286270b0…` / `326099f08e24…`）在**仓库存储格式与工作树里都是裸 LF**，是本笔之前既存的历史格式偏差。本笔仅在其后追加行，并**断言写入前后裸 LF 计数均为 2**（未引入新偏差）。**记为已知项**：将来做「CRLF 全仓归一」时应一并处理，属缺陷 7 lint 的同类面。

## B-44（`#294` 协议口径修订：B-43 报告缺陷 1 / 2 / 3 / 4 / 6）

**状态**：7 笔（交付 2：`f162534` 五条口径一次落 → `3bd2529` 缺陷 1 实施边界如实化；落点 1：`b074740`；台账 3：`e0662c5` / `ecf5a6c` / `b1c9dac`；修复 1：`c39f462`）。**纯 docs**。**已推送（`607dee4..b1c9dac` ff，Gate-0 6/6）/ 已关单**（`closedAt 2026-09-23T09:08:30Z`，comment [#issuecomment-5792100712](https://github.com/EricKingWhy/intelligence-agent/issues/294#issuecomment-5792100712)）。
**依赖关系**：无前置；与 `#295` 是配套票（口径在 `#294`、实现与三个控制样本在 `#295`）。

**问题**：B-43（`#286`）实测报告 §3 登记 7 条体系缺陷，其中 5 条是**纯口径类** —— 协议写得不够细，导致合理行动被误判成越界，或反之：

1. **P3 无廉价修复通道**（最重要）：`src/**/*.py` 不在 `DOC_PATTERN` 内 ⇒ 任何 `src/**` 改动（**哪怕只改一行注释**）都让冻结树失效、须重跑全量 + 修后重审 ⇒ 理性选择是推迟修复，**已知缺陷随交付进 main**（B-43 的实际结局：`provider.activate` docstring 对 `max_depth=0` 描述夸大，两轴各自独立报出，因代价过高未就地修）。
2. **「红证」与「变异」同词不同目的**：红证 = 改动前树红 / 改动后树绿（证「用例能发现实现缺失」）；变异 = 在改动后树破坏实现 ⇒ 用例转红（证「用例有鉴别力」）。B-43 实测 M1 / M2 命中**完全相同**的三条用例 ⇒ 单看任一条都会误判「其余检查也被覆盖」。
3. **safe-delete 假红污染每次全量读数**：shim 的跨测试删除配额（`threshold: 50, scope: turn`）后期耗尽 ⇒ `tests/evaluation/*` 假红（B-43 实测 `3124 / 3 failed` 全为该形状）。既已登记、处置脚本也在仓库，但**默认跑法仍走 shim** ⇒ 每批仍要人工归因一轮。
4. **`git archive` 副本行尾是 CRLF ⇒ 变异锚点静默 0 命中**：`core.autocrlf=true` ⇒ 副本里 `src/**` 是 CRLF（实测 `depth.py` 含 `b'\r\n'`），用 `\n` 写多行锚点 ⇒ `.replace()` 零命中 ⇒ 产生「对照绿 / 变异也绿」的**假读数**，与「用例无鉴别力」**同形**。
6. **本次未覆盖 §8.8.3 失败回退路径**：P0/P1 = 0 ⇒ 回退表**一行都没被走到**，不能据此声称「回退表已验证」。

**交付面**：

1. `f162534` `docs(#294):` —— 五条口径一次落（§8.1 第 3 条缺陷 1 例外 + §8.8.2 INV-1 指针 + §8.3 第 4 条缺陷 2 分列 + §8.6 缺陷 3 默认跑法 + §8.1 第 6 条缺陷 4 行尾 + §8.8.3 缺陷 6 登记与附表）。
2. `3bd2529` `docs(#294):` —— 缺陷 1 的实施边界如实化（判据口径在本票、**实现与三个控制样本归 `#295`**）。

**关键设计（为什么这样写）**：

- **判据只增不减**：缺陷 1 的新例外**不修改**判据① 对 `DOC_PATTERN` 的定义，而是**并列新增**；§8.8.2 INV-1 只补一句**指针**、不复制条文（单一事实源）。
- **缺陷 6 的登记处不选 `verification.map.tsv`**：那张表的语义是「代码面 ↔ 必跑车道」映射、且有形状守卫（`tests/test_verification_map.py`），回退路径覆盖是**另一个维度**，塞进去会同时破坏两边（守卫拒 + 语义错位）⇒ 登记改成 §8.8.3 的**本节附表**。
- **缺陷 3 的执行边界写进条文**：若 `env_failures` 结构化字段需要改 `scripts/gate0.py`，则**必须移入代码面票**；本票只写口径与判据。

**实测读数**：

- **判据只增不减（票面验收第 1 条）**：`git show HEAD:docs/SDD_WORKFLOW_PROTOCOL.md`（改动前，735 行）的 **707 条非空行**在本版本中**一条不缺**（逐行子串比对，缺失 0）。
- 协议 **735 → 803 行**（+68），文件 **76016 → 83581 B**；纯 CRLF，**裸 LF = 0**（写入前后均断言）。
- 12 个内容锚点全部命中（缺陷 1 / 1-判据自证 / 1-兜底 / 2 / 2-反例 / 2-共用失败面 / 3 / 3-env / 4 / 4-命中数 / 6 / 6-B42）。
- `tests/test_verification_map.py` ⇒ **20 passed**（证明没碰 map 的形状）。
- `git diff --check` ⇒ **rc=0**。

**残余（登记，不阻断）**：

1. **缺陷 1 的判据尚无实现**：本票只写口径；`scripts/check_semantic_equiv.py` 与三个控制样本（负控 / 正控 / 删真语句反控）在 `#295`。**在 `#295` 落地前，该例外不得被实际引用**（判据不可执行 ⇒ 引它等于自造通道）。
...
## T1（`#306`，B 链首票）：长任务执行边界契约冻结 —— ADR-0044 + 六章正式规格（2026-09-25 · 已合入 `main`（PR #328 → merge `7989e185`，2026-09-26）、已关单）

**状态**：✅ 交付与审查闭合；**零** `src/**` / `tests/**` / `web/**` 改动（纯契约与文档）。分支
`zcode/T306-longrun-budget-contracts`（交付 tip `ec4f73e`；集成前按 §14.9 回补 `origin/main` 117 笔后为 `c3657dc6`），
固定点 `d0e2dcb`（= 施工开始时的 `origin/main`）。
**集成（2026-09-26）**：经 **PR #328** 合入 `main`（merge `7989e185`）；服务端 `gate0` 绿；该 merge 与分支 tip
**同树**（`530de5dfcac8`）⇒ 覆盖闸门按「零新增内容」自动归属、无需台账行；真 `main` 上复跑覆盖闸门 exit 0。
PR 对 `main` 的净贡献 = **19 文件 / +2192 −31，全部在 `docs/**` + `goal/**`**（`git diff --name-status 26a4c2b5 7989e185`
排除这两棵树后为空 ⇒ **代码面零变更**）。**已按 §14.12 关单**（issue comment 附分支 / commit / 门禁 / CI 证据）。

**票面**：GitHub `#306`；父票 `#305` 的产品与架构裁决已冻结。要求：把 #305 的 FIXED 决策写进正式规格、
给出决策覆盖表，且不得把未验证数字写成正式要求。

**交付序列**（9 笔交付 + 台账 / 读数 + 回补 / 归一化 / 订正 + 集成）：

| # | commit | 内容 |
| --- | --- | --- |
| 1 | `c4a696e` | 摄入 #305 正文副本（38,483 字符 / 41,041 字节；逐行一致，唯一差异是正文末尾多一个换行；**行尾硬换行双空格已由 `0b7d47c` 归一化** —— 见残余 3） |
| 2 | `002292b` | 摄入决策依据调研 |
...
| — | `ec4f73e` | Gate-0 读数落盘 |
| 10 | `a2a031de` | **§14.9 回补**：`origin/main`（117 笔）→ 本分支；两份 docs 索引冲突按**并集**解析（T1 在前、T2–T5 在后），归档行号按合并后**实测**重算（`26a4c2b5` 已是父二 ⇒ 无第三方内容进入） |
| 11 | `e56ade18` | Gate-0 裸全量落盘（回补后树 `feb5e54c591c`，6/6 PASS，18.8s） |
| 12 | `0b7d47c1` | **修复**：两笔摄入副本的行尾硬换行双空格**归一化**（CI `gate0` 车道 ① 抓回的第三例「同一份脚本 ≠ 同一个环境」；61 行 / 122 字符，内容层等价 —— 见残余 3） |
| 13 | `7bb6c8ed` | **订正**：两处「与 #305 正文逐行一致」的**自加**保证改为「内容层逐行一致」（同一新证据的连带订正） |
| 14 | `c3657dc6` | Gate-0 裸全量落盘（最终树 `d1d7ba30cf83`，6/6 PASS，17.8s） |
| 15 | `7989e185` | **集成**：PR #328 → `main`（与分支 tip 同树 ⇒ 零新增内容） |
...
- **集成前补测（文档面重测；代码面读数按 §8.1 继承）**：本票**无代码面** ⇒ `git diff --name-status 847b218e 7989e185 -- src tests web scripts evaluation .github`
  只有 `M scripts/live_gate.py`（**模式位**，`2d3761c`，内容零变更）⇒ 权威代码读数沿用 T5 冻结树 `847b218e` 的
  `1 failed / 4163 passed / 2 skipped / 50 deselected`（唯一红 = `tests/memory/test_memory_v2_recall_dataset.py` 的 CRLF 可移植性缺陷 —— **跨票既有环境项**，登记在 T5 段残余 5）。
  文档面在最终树上重跑裸全量 Gate-0：回补后树 `feb5e54c591c` **6/6 PASS**（18.8s ⇒ `docs/gate/a2a031deeac6b50404095a821cb012c14f6b2163.json`）、
  归一化 / 订正后最终树 `d1d7ba30cf83` **6/6 PASS**（17.8s ⇒ `docs/gate/7bb6c8eda1c4f866ad4b540e25a167e00fc399cd.json`）。
- **CI 一致性复核**：本地复跑服务端同一车道（`scripts/gate0.py --since 26a4c2b5 --only diff-check`）⇒ PASS（19 文件）；
  PR #328 服务端 `gate0` 绿。本地**裸全量**照不到该车道的这一范围（成因见残余 3）。

**残余（登记，不阻断）**：

1. **前向失配 —— 已由 `#312` 闭合（2026-09-26 复核）**：原登记「ADR-0033 与跨仓契约仍把
   `max_steps_exceeded` 写成撞保险丝时的失败归因面」。T4 落地时按本条的预告**同步了两份文档**：
   ADR-0033 §4 新增「（`#312` 追加）预算到顶不是失败」条，并把该行标注为「只对 `#312` 之前的历史载荷成立」；
   `docs/BACKEND_CONTRACT_STREAMING_UI.md` §4 同条改为删除线 + 「运行期已不可达」。实现侧同证：
   `src/agent_harness/agent/runtime.py:2031` 记「该受控失败终态已不可达并随之删除」、
   `memory/v2/eligibility.py:52` 同步移出白名单（`src` 全库 `git grep max_steps_exceeded` 只剩这两处注释）。
   ⇒ **无需用户裁决**，登记闭合（同步笔 = T4 的 `0fdd197`）。
2. `#305` 正文 FIXED 段自述 "All 30 user decisions" 与其 35 条 User Stories 不一致（上游自身不一致；只登记）。
3. **范围级 trailing whitespace 已处置（`0b7d47c`）**：原判「61 行落在两笔摄入副本（PRD 59 / 调研 2）、
   是 GitHub 正文的 Markdown 硬换行双空格 ⇒ 保留并登记」被**新证据推翻** —— `main` 受保护后 CI 的 `gate0`
   以 `--since <PR base>` 跑车道 ①，覆盖**整个 PR 区间** ⇒ 它从「不阻断的痕迹」变成**硬阻断**（PR #328 实测 FAIL；
   本地照不到的成因 = 裸全量不给 `--since` 时车道 ① 只覆盖工作树、pre-push 的范围又只从远端分支 tip 起算）。
   处置 = **归一化内容**：`git diff --check` 判据与 `.gitattributes` 均未改（门禁一条不减）。
   等价性判据两条可复跑：剥行尾空白后逐行逐字节相同（61 行、每行恰好 2 个空格）+ `git diff --ignore-all-space` 为空。
   「与 #305 正文逐行一致」是**自加**保证、非 `#306` 的 AC ⇒ 随之订正为「内容层逐行一致，唯一差异 =
   末尾多一个换行 + 61 行行尾硬换行双空格已归一化」。
4. 规格内 `max_delegations` 默认值在 `02 §5.1` / `10 §5.1` / `11 §6.1` 各一次，按「每章写自己作用域
   的契约」保留；ADR 侧已清零。

**下一步**：本票**已集成、已关单**；B 链其后各票（`#314` = T6 → … → `#320`）由本线按交接册顺序继续。

## T2（`#307`，B 链第二票）：真实模型 + 生产工具 Live Gate 证据运行器（2026-09-25 · 已合入 `main`（随 PR #326 → merge `fd50af4b`，2026-09-26）、已关单）
...
4. **已关单（2026-09-26，随 PR #326 合入 `main` 后按 §14.12 关闭）**。

## T3（`#308`，B 链第三票）：local 轮次预算 `budget.local.max_agent_turns` 契约落地（2026-09-25 · 已合入 `main`（随 PR #326 → merge `fd50af4b`，2026-09-26）、已关单）
...
5. **已关单（2026-09-26，随 PR #326 合入 `main` 后按 §14.12 关闭）**。
6. 后续票按交接册顺序：`#312`（T4）→ `#313` → `#314` → `#315` → `#316` → `#317` → `#318` → `#319` → `#320`。

## T4（`#312`，B 链第四票）：预算到顶落非终态 `run/paused`、同一 run 带 CAS 续跑（2026-09-25 · 已合入 `main`（随 PR #326 → merge `fd50af4b`，2026-09-26）、已关单）
...
9. **已关单（2026-09-26，随 PR #326 合入 `main` 后按 §14.12 关闭）**。
10. 后续票按交接册顺序：`#313` → `#314` → `#315` → `#316` → `#317` → `#318` → `#319` → `#320`。
## T5（`#313`，B 链第五票）：Provider 请求 / token / cost 计入 run 预算并可按四维暂停续跑（2026-09-26 · 已合入 `main`（PR #326 → merge `fd50af4b`）、已关单）
...
**已完成（2026-09-26）**：T1 经 PR #328 → merge `7989e185` 落 `main`，本节 ④ 的「悬空指针」随之消失。
...
**已闭环（2026-09-26）**：T1 经 PR #328 → merge `7989e185` 合入 `main`，指向 `ADR-0044` / `02 §5.1` 的「机制单点」指针恢复有效。
...
出发基点 `718ab562`（= T5 记录笔之后的 `origin/main`）。**代码冻结树 = `317cf2cb` / tree `2b906371`**
（其后只有证据 / 台账 / 记录三类 docs-only 提交）。**待集成**（PR → `gate0` → merge，§14.4 通道），未关单。
...
**真实 Live Gate（真模型 + 生产工具，同一 SHA `317cf2cb` / tree `2b906371`）**：

| 场景 | 判定 | 读数 |
| --- | --- | --- |
| `long-task-past-legacy-turn-limit`（v3） | PASS 3/3 | 3 次尝试 `steps=16/16/16`（均越过旧上限 10）、工具 16/16/16、32.8s / 33.8s / 34.6s；`tool_calls_counted_once`：逻辑调用=16 ×3（`tool/call`−准入前被拒=0），per-tool 表 `bash=12/14/12` + `read=3/1/3` + `write=1/1/1`；`tool_attempts_counted_separately`：`tool_calls=16 = tool_attempts=16`（本 run 无 retry ⇒ 相等，判据明写"retry 只会让 attempts 更大"）；`tool_quota_explicit_and_scoped`：请求的 `{'bash': 24}` == `run/started` 落盘快照，bash 已接纳 12/14/12（剩余 12/10/12，**未撞线**）、`write` 未配 ceiling 但已接纳=1（未配 ≠ 不计数）；`production_tools_used`：`bash→agent_harness.tools.bash.BashTool` 等三条实现路径全落在 `agent_harness.*`，带接纳点增量的 `tool/result` = 16/16（畸形 0、名字不一致 0）；`tool_accounting_replay_stable`：复读账本 == 首读账本（含两张表与 ceiling） |

证据目录 `docs/live_gate/20260926T012418-317cf2cb02c9-long-task-past-legacy-turn-limit/`（`validate` 复核
**24/24 条自洽**，含工作树偏离、沙箱销毁、凭证扫描 0 命中）。⚠ 该场景**不证明撞线与恢复**——判据里
`bash 已接纳 < ceiling` 明写"未撞线"，撞线 → 暂停 → 恢复那条链由 `budget-pause-resume-same-run`
