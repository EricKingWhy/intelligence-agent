...
- 2026-09-19：**T09 / #254 Recovery lock-outside adjudication**（实现 `923e72c`）。`RecoveryCoordinator` 在 sidecar 恢复锁内完成 Ledger-first load、确定性合成、RUNNING→UNKNOWN→NEED_RECONCILE 与 `operation/reconcile-required`；`ReconcileCallback.resolve()` 锁外等待，重锁后复读 Ledger/事件并复用 #253 token 拒绝 stale verdict，只有匹配快照才写终态与配对 `tool/result`。timeout/cancel/exception 不写虚假完成；并发最终至多一个 durable outcome；确定性 recovery 可在另一会话 callback 阻塞时完成。专项 + Bash integration 57 passed；ruff clean；非 web 2187 passed / 10 skipped / 42 deselected；web `-x` 371 passed；diff clean、uv.lock 未变；按 v2 单票不自审，待下一票批量审查。#254 / 父票 #242 保持 OPEN。详见月度归档。
- 2026-09-19：**T10 / #255 Read-only Catalog router seam**（实现 `d06a635`）。六个只读 Catalog/Profile 端点迁入 `web/catalog.py` 的 FastAPI `APIRouter`，以 `Annotated[..., Depends(get_catalog_state)]` 注入窄 `CatalogState` Protocol；`create_app` 显式一次注册，保留 app 常量兼容导出，未触及 SSE/WebSocket/审批/Session 生命周期。独立 fake-dependency/OpenAPI smoke + Catalog 专项 47 passed；OpenAPI baseline/after 路径顺序、operationId、description、参数、响应 schema 逐端点一致；非 web 2187 passed / 10 skipped / 42 deselected；web 373 passed；ruff/diff clean、uv.lock 未变；按 v2 单票不自审，待 #254/#255 批量审查。#255 / 父票 #243 保持 OPEN。详见月度归档。
