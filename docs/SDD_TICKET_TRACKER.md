...
2. **协议既存悬空引用 `§8.3.1`（L295）**与 **§8.3 标题未覆盖第 7 条** —— 审计包 §7 表已登记，**不在本票范围**（留给批 3 / `#293`）。
...
5. **`5db3d43` 三层 Scope 偏离**仍待用户裁决（与本票无直接依赖，登记备查）。

---

## B-41（`#292` 增量验证：验证映射表 + 守卫 + `gate0 --affected`）

**状态**：6 笔代码/测试 + 5 笔文档 + 落点已完成，**覆盖闸门 exit 0 后集成**（2026-09-22）。**拟归批次 B-41**（本段即登记）。父票面 = GitHub `#292`。
...
**门禁 / 覆盖闸门 / 集成**：
- Gate-0（推送前）：见本轮落点记录；`.githooks/pre-push` 自动跑全量 6 车道。
- 覆盖闸门：本批 11 笔的 ❌ 由 **11 → 0**；台账新增 **2 条审查行**（覆盖代码面 6 笔）+ **1 条白名单**（`e62005e`，docs-only）。
- 集成：`push origin main`（fast-forward）+ 按 §14.12 关单 `#292`；推送前后 `git ls-remote --heads origin` 实测。

**索引按归档实测重算（本批落点顺带修正）**：落点前实测 `^- 2026-` = **304**、`^- 2026-09-22` = **9**、09-22 区间 = **L628..L753**，而 `docs/PHASE_STATUS.md` 旧写 **303 / 8 / L628-736** ⇒ **B-40 落点未随归档更新该行**（B-40 在 L738-753）。本批按实测重算为 **305 / 10 / L628-L767**，并记 B-40 子项 **15** 条、B-41 子项 **13** 条。**同一轮还逮到第三处错误索引**：既有 B-33/B-34 条目把 B-34 写成 `L645`（**实为 `L650`**；`L645` 是 B-33 条内的深缩进子项）⇒ 已一并更正。
