...
- 2026-09-19：**T07 / #252 Session single durable write funnel——实现 + 行为回归 + 结构护栏**（实现 commit `0851c43`，2 文件 +100/−4）。**做了什么**：`Session.append` 与 `adopt_history` 保留各自的词表校验和事件构造语义，统一通过私有 `_persist_event` 完成 `store.append_event → _events.append → _next_seq += 1`；只有 normal append `notify=True`，fork seed adoption `notify=False`。新增 AST 护栏锁住 Session 类内只有该 helper 直接写 Store，两个入口都调用同一 funnel。**行为保持**：#251 冻结的失败不推进 seq/内存、先校验、落盘后 listener、adopt 不通知、逐字保留 seed 身份字段、逐条写入保留已落盘前缀全部通过；未改 Store/fsync、seq conflict、hard-delete、recovery/旁路公开入口或 adopt_history 为 append 循环。**红证**：既有 #251 golden + fork/store/recovery 回归；结构护栏若任一入口重新直接调用 Store 或漏掉共享 helper 即红。**门禁**：`ruff check .` All checks passed；#252 专项（write_funnel + write_behavior_golden + fork）34 passed；session/recovery/memory 回归 415 passed；后端 `pytest --ignore=tests/web -q` **2168 passed / 10 skipped / 42 deselected / 0 failed（4:53）**；`pytest tests/web/ -x -q` **371 passed / 0 failed（3:11）**；`git diff --check` 干净；`uv.lock` 未变。**覆盖闸门**：按 v2 单票不自审，待 B-17 累计 2–3 票批量审查补台账；#252 / 父票 #241 保持 OPEN。**已知边界**：`tests/web/` 不带 `-x` 串跑的环境红仍按 B-16 归属，不计入本票。
- 2026-09-19：**T08 / #253 Recovery adjudication token contract——实现 + 红证 + recovery 回归**（实现 commit `42b7faf`，3 文件 +149/−2）。**做了什么**：新增 `RecoveryAdjudicationToken` frozen/slots value object，携带复合身份 `session_id + operation_id`、`OperationState` 与固定 SHA-256 state fingerprint；从 `Operation` 快照确定性构造并以完整 token 比较。**安全边界**：args/result 等可能含 secret 或大 payload 的字段不进入 token；依赖现有单调状态机，未伪装成持久化 revision；不改 `Operation`/SQLite schema、`OperationLedger`、`ReconcileCallback.resolve()`、UNKNOWN/NEED_RECONCILE 词汇或 coordinator 锁流程，#254 的锁外等待/重锁 stale 拒绝仍未实现。**红证**：基线缺口为包级 `RecoveryAdjudicationToken` ImportError；变异覆盖 immutable/slots、固定 digest、session/tool identity、所有状态变异失效、args/result 不入 token、UNKNOWN 与 NEED_RECONCILE 区分。**门禁**：#253 recovery/coordinator/scan/reconcile 专项 **59 passed**；`ruff check .` All checks passed；后端 `pytest --ignore=tests/web -q` **2180 passed / 10 skipped / 42 deselected / 0 failed（5:05）**；首次 Web `-x` 串跑在 224 passed 后遇 SSE 断流，精确用例单跑通过后完整重跑 `pytest tests/web/ -x -q` **371 passed / 0 failed（3:14）**；`git diff --check` 干净；`uv.lock` 未变。**覆盖闸门**：代码提交按 v2 留待 B-17 累计批量审查，未加白名单；#253 / 父票 #242 保持 OPEN。
