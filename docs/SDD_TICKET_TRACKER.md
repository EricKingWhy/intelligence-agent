...
| # | 轴 | 级别 | finding | 处置 |
| --- | --- | --- | --- | --- |
| 1 | Standards | **P1** | 审查行的 fixed point `45744d3` 是本批**自己的开篇提交**，不在任何审查行范围内 ⇒ 闸门判「未审查且未声明」（与协议 §5 第 8 条点名的 #213 同族失效形态：**fixed point 错位即静默豁免一票**） | 台账 `[whitelist]` 补 `45744d3`（docs-only，机械可验）；**不改** fixed point——往前挪等于谎称审过它的 diff |
