...
- 2026-09-17：**#221 门禁 + 集成 + 关单**。门禁：`tsc -b` 0 错 / vitest **943 passed · 55 files** / oxlint **0 error · 44 warning** / playwright **418 passed · 0 failed（9.7m，--workers=2）** / `git diff --check` 干净。**含一条自我勘误**：先前写下的 playwright「410 passed · 0 failed」是 `404 + 6` 的**推算值**，实测该树为 **412 passed · 4 failed**（四条单独复跑全绿、且都不经过本票改动的路径 ⇒ 满载抖动），补 T12q 后 418 全绿。后端未重跑（本票零 `src/` 改动，如实登记）。集成：集成区 `D:\intelligence-agent` `merge --ff-only` → `push origin main`（§13.4/§14.4）。**关单未执行：本机 GitHub 不可达**（代理拒连 + gh keyring token 失效，实测），待恢复后按 §14.12 补 `gh issue close 221`。
