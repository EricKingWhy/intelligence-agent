...
**状态**：`523efc1` 之后 5 笔——`35268a3` 主体 → `a6c34ce` / `ca2a3d0` 审查 findings 修复 → `37c01df` 台账行 → 末笔补 B-30 自身记录（本节 + `PHASE_STATUS` 索引与按日表 + 归档明细）。**逐条明细唯一写在 `docs/phase_status/2026-09.md` 的 B-30 段**；审查范围行在 `docs/review_ledger.tsv`；本节只留票面状态、约定与残余。

**变更面**：① `AGENTS.md` **907 → 822 行**——§4.1/§4.2 清单下沉 `docs/agents/review-debug-playbook.md`、§9.5/§9.6 展开说明下沉 `docs/agents/implementation-discipline.md`、§15 主题规则以 `web/PRODUCT.md` 为权威（§15 保留编号 + 硬规则 + 指针）、§16.2 / §13.1 / §13.3 改为指针；**58 个标题（含 48 个编号节）编号与标题逐字未变**（机检）。② `docs/PHASE_STATUS.md` **132 行 / 62.2 KB → 86 行 / 23 KB**。③ 新建 `docs/README.md`（`docs/` 唯一导航入口）。④ **75 个文件 `git mv` 进 `docs/archive/`**（58 份 integration-prompts + 17 份 handoffs；顶层 .md 147 → 73；**不留 stub**，映射在两个目录 `README.md`）。⑤ 引用同步 55 文件 / 184 处（含 `web/**` 注释与 `tests/web/test_web_amend_validation.py` docstring）。
...
**门禁与读数传递（协议 §8.1，本批是该条第二次实际使用）**：后端全量只在冻结树 `35268a3` 上跑**一次**（`2902 passed / 13 skipped / 42 deselected`；较上一冻结树收集数一致，多出的 11 个 skip = **Docker daemon 未启动**，非用例丢失）；其后 `35268a3..本批 tip` **只有 docs / 台账改动**（判据①：`git diff --name-status --no-renames` 全为 `M` 且逐条命中 `DOC_PATTERN`；判据②：`git status --short` 仅剩 `?? .zcodeignore`）⇒ 后端读数传递到被集成树。**前端**：初跑时 `web/node_modules` 是指向 `D:\intelligence-agent-frontend\web\node_modules` 的 junction、其依赖树缺 `jsdom` ⇒ 8 个 vitest worker 无法启动（982 条用例"实际通过"但**未计为通过**）；按本仓 `pnpm-lock.yaml` 重建依赖后复跑：`tsc -b` **0 错** / vitest **65 文件 1040 用例全过** / `oxlint` **0 error · 42 warning** / `vite build` 成功 / playwright e2e **436 passed**。`ruff` clean；覆盖闸门 exit 0。
...
③ **陈旧 `AGENTS.md` 子节引用（独立复扫 8 处）**：`§13.1.3` / `§16.4` / `§16.6` 等在 `523efc1` 与当前 `AGENTS.md` 里**都不存在**（既有，非本批引入——Codex 审查轮登记为 13 处，接手方按"`AGENTS.md` + `§x`"归因复扫得 249 处引用中 8 处不可解析），且全部落在历史语境（归档件、`docs/integration/*`、tracker 历史条目、`FRONTEND_ISSUES_LOG` / `T9_TURN_INDEX_DELIVERY`）⇒ 按 Q13 **不改写历史正文、只登记**。
