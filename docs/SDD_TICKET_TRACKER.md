...
**状态**：**已落地并本地验证，尚未推送**（分支 `gate/zero-content-attribution`，四笔提交 `da7dfef1..3bd597be`）。承接 B-46 残余①那条已登记的结构性缺陷（"每个 PR 合入都会在 `main` 顶端重造一个 ❌"），按用户裁决**单独开一个 PR** 处置。**无独立 GitHub issue** —— 它是 B-46 已登记残余的实现票，不是票面驱动。
...
**交付（4 笔）**：`9367a57e` 判据 + 接线 + `resolve_trees` 形状校验；`583e867a` 首审处置；`d3c9373d` 文档同步（协议 §7 第 8 条 / 审计 §5.2、§9.4.4 / `.github/workflows/gate0.yml` 头部注释 —— 后者正是 B-46 刻意推迟到本批的那一处）；`3bd597be` 修后重审处置。测试 `tests/tooling/test_review_coverage_zero_content.py` 9 条 = 7 条判据层 + 2 条**合成仓库**端到端：正控（树 == 父 2 的树 ⇒ `exit 0` 且打印 `✅ 零新增内容`）**修前是 `exit 1`**；反控（`--cc` 空表但树 ≠ 任何父 ⇒ **仍须** `exit 1`）拦住「凡是空表就放行」的假实现。端到端用合成仓库而不硬依赖本仓那笔真实 merge：后者迟早被补上台账行、不再进入判定循环，硬依赖它的测试会**静默失效**。
...
**读数**：`ruff check .` 全绿；`tests/tooling/` **154 passed**；覆盖闸门落行**前**本仓区间唯一 ❌ = 本批 4 笔（提交总数 717 / 已审查 465 / 待判定 252，`exit 1`），落行**后** 465 → **469**、252 → **248**、**exit 0**；Gate-0 裸全量 **6/6 PASS**（读数落盘 `docs/gate/<本批台账提交 sha>.json`，见同批 docs 读数列）。
