...
**代码冻结树 = `847b218e` / tree `b295b582`**（其后只有证据 / 台账 / 记录三类 docs-only 提交）。
...
**权威 ruff 车道（`ruff check .`）的 27 条 findings —— 本票唯一一次"被门禁抓回来"（如实记录）**：
...
不需台账行；最终 PR 合并因分支已含 main 顶端 ⇒ 与父提交二**同树** ⇒ 按"零新增内容"自动归属。④ T1 目前**卡一处
机械冲突**：它的台账行 `300-d0e2dcb-ec006c7` / `301-ec006c7-daaf662` 与 main 上既有 300 / 301 **同编号不同内容**
⇒ T1 自己的收口需要先改号（属 T1 的范围，本票只登记）。
