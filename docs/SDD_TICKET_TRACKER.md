...
| --- | --- | --- | --- | --- |
| `tests/sandbox/test_docker_sandbox.py::test_timeout_stops_late_workspace_mutation` | **失败断言行 = `:93` `assert "before" in result.stdout` 且 stdout 为空**，`duration_ms` 超预算（1.0 s；实测 `2092.9` = 容器冷启动吃满）；**同用例其它断言（`:92` / `:94` / `:96`）失败一律按真缺陷阻断**（`:96` 断的是"超时后迟到写入仍发生"，即沙箱边界本身） | 关键读数落在版本控制内：本表即读数（`1 failed, 2912 passed`、623.91 s）与 `.workbuddy/full_264_fix.log`；同树单跑 3 次 **2 通过 / 1 失败**（pass / fail / pass）；最终树 **0 failed**（516.19 s，`.workbuddy/full_264_final.log`——日志路径只作旁证，`.workbuddy/` 被 gitignore） | **无法取得（2026-09-21 失败尝试，如实登记）**：独立审查子代理在冻结树 `45a367f` 上复跑 3 次，**3/3 `1 skipped in 0.15–0.19 s`**（`SKIPPED: Docker SDK or daemon is unavailable`，`tests/sandbox/test_docker_sandbox.py:80`）；`docker info` ⇒ `failed to connect to the docker API at npipe:////./pipe/dockerDesktopLinuxEngine`。**本机没有 daemon ⇒ 该时序签名在本机不可能复现，任何次数都无意义** | ⚠ **暂不生效**：未取得独立确认前仍按「阻断、需完整重跑」处理。**待办（不阻断，但会以"阻滞集成"的形式反复出现）**：该条要生效，独立确认必须在**带 Docker daemon 的环境**上做（本机做不到）；在那之前每次全量撞上它都得整批停下等一次带 Docker 的复现。**同 hazard 的兄弟用例未登记**（`:100-112` 的同 budget detached 版注释自述"预算/延迟同本条"）：理由是它**尚未实测失败**——一旦出现同签名失败，先按阻断处理再决定是否登记 |
