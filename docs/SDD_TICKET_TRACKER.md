...
**状态**：`#264` 的实现与证据已完成、**待集成关单**。交付面 = `src/agent_harness/agent/runtime.py`（六个 run 终结点提取为 `_TerminalArms` + 五个 `_terminal_*` 方法，`_drive` 仍是唯一 loop owner）+ 新增 `tests/agent/test_terminal_arms.py`（13 条臂契约用例）。**行为零变化**的判据是 #263 的 golden：`test_event_sequence_golden.py` 本批**逐字未改**（blob `d0c868e120f8` 两侧相等）且全绿。
