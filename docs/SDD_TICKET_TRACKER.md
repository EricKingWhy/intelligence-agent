...
| --- | --- | --- | --- | --- |
| `tests/sandbox/test_docker_sandbox.py::test_timeout_stops_late_workspace_mutation` | **失败断言行 = `:93` `assert "before" in result.stdout` 且 stdout 为空**，`duration_ms` 超预算（1.0 s；实测 `2092.9` = 容器冷启动吃满）；**同用例其它断言（`:92` / `:94` / `:96`）失败一律按真缺陷阻断**（`:92` 断言 `exit_code == -1`、`:94` 断言 stderr 含「超时」、`:96` 断言**迟到写入没有发生**——即"超时后 marker 仍不存在"，失败 = 沙箱边界被突破） | 关键读数落在版本控制内：本表即读数（`1 failed, 2912 passed`、623.91 s）与 `.workbuddy/full_264_fix.log`；同树单跑 3 次 **2 通过 / 1 失败**（pass / fail / pass）；最终树 **0 failed**（516.19 s，`.workbuddy/full_264_final.log`——日志路径只作旁证，`.workbuddy/` 被 gitignore） | **尚未取得（2026-09-21 尝试，如实登记）**：独立审查子代理在冻结树 `45a367f` 上复跑 3 次，**3/3 `1 skipped in 0.15–0.19 s`**（`SKIPPED: Docker SDK or daemon is unavailable`，`tests/sandbox/test_docker_sandbox.py:80` 的 `@docker_required`）；`docker info` ⇒ `failed to connect to the docker API at npipe:////./pipe/dockerDesktopLinuxEngine`。**注意：这是 daemon 没启动，不是机器做不到**——Docker Desktop 已安装（`D:\Docker\resources\bin\docker`、context `desktop-linux`），同日 B-28 还跑出过真容器（`duration_ms=2092.9`） | ⚠ **暂不生效**：未取得独立确认前仍按「阻断、需完整重跑」处理。**待办（可当场消除，不是环境限制）**：**启动 Docker Desktop 后当场补跑 3 次**即可取得独立确认并翻生效；在那之前每次全量撞上它都得整批停下等一次复现。**同 hazard 的兄弟用例未登记**（`:100-112` 的同 budget detached 版，且它**没有** `:93` 那条断言 ⇒ "`:93` + stdout 空"签名在它身上不可能出现，不登记是对的）：一旦它出现**自己**的同签名失败，先按阻断处理再决定是否登记 |
...
**状态**：`523efc1` 之后 6 笔——`35268a3` 主体 → `a6c34ce` / `ca2a3d0` 第一轮两轴 findings 修复 → `37c01df` 台账行 → `93de00e` 接手补齐（本节记录 + `PHASE_STATUS` 索引与按日表 + 归档明细，并修 4 处旧判据 / 悬空指针） → 末笔（第二轮两轴 10 条 findings 处置，见下「修后复扫」段；sha 见本批「集成」段）。**逐条明细唯一写在 `docs/phase_status/2026-09.md` 的 B-30 段**；审查范围行在 `docs/review_ledger.tsv`；本节只留票面状态、约定与残余。

**变更面（逐条枚举见归档 B-30 段「交付面」）**：① `AGENTS.md` **907 → 822 行**（`523efc1` → `35268a3`；末笔按 §13.4 逐字还原后 `93de00e` = 836 行）——§4.1/§4.2 清单下沉 `docs/agents/review-debug-playbook.md`、§9.5/§9.6 展开说明下沉 `docs/agents/implementation-discipline.md`、§15 主题规则以 `web/PRODUCT.md` 为权威、§16.2 / §13.1 / §13.3 改为指针；**58 个标题（含 48 个编号节）编号与标题逐字未变**（机检）。② `docs/PHASE_STATUS.md` **132 行 / 63.5 KB → 85 行 / 23.1 KB**（同两节点；按 `git cat-file -s` 对象字节，末笔 88 行 / 24.1 KB）。③ 新建 `docs/README.md`（`docs/` 唯一导航入口）。④ **75 个文件 `git mv` 进 `docs/archive/`**（顶层 .md 147 → 73；不留 stub，映射在两目录 `README.md`）。⑤ 引用同步 55 文件 / 184 处。
...
③ **陈旧 `AGENTS.md` 子节引用（既有，非本批引入）**：按固定规则复扫（规则=**同一行**内同时含 `AGENTS.md` 与 `§x.y`，编号对照 `AGENTS.md` 的 `#`–`####` 标题集；扫 `git ls-files` 全部文本文件）得 **269 处引用 / 42 个不同编号**，其中 **4 个编号在 `AGENTS.md` 里不存在**（`§2.4` / `§13.1.3` / `§16.4` / `§16.6`）、共 **12 处**（含本条残余自身举的 2 个例子）：`docs/phase_status/2026-09.md:292` / `:571`、`docs/SDD_TICKET_TRACKER.md:703` / `:3875`、`docs/FRONTEND_ISSUES_LOG.md:1875`、`docs/T9_TURN_INDEX_DELIVERY.md:145`、`docs/integration/FRONTEND_INTEGRATION_PROMPT.md:316`、`docs/integration/MERGE_EXECUTION_ORDER.md:191`、`docs/integration/FRONTEND_RECOVER_FORK_SCROLL_INTEGRATION_PROMPT.md:163`、`docs/archive/handoffs/HANDOFF_AGENTS_MD_CONSERVATIVE_AUDIT.md:286` / `:691`、`docs/archive/integration-prompts/INTEGRATION_PROMPT_FRONTEND_FIXES_ROUND11.md:13`。**这 4 个编号在 `523efc1` 的 `AGENTS.md` 里同样不存在** ⇒ 全为既有；且都落在历史语境（归档件、`docs/integration/*`、tracker 历史条目、设计稿、前端测试注释）⇒ 按 Q13 **不改写历史正文、只登记**。（本批两轮里出现过的 8 / 13 / 249 三个数字来自不同抽取窗口、**不可复现故撤回**，以本条规则为准。）
