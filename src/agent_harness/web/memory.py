...
from agent_harness.memory.v2.commands import trusted_identity_for_session
        ...
        for event in events:
            ...
            for raw in raw_memories:
                ...
                explanations.append(MemoryRecallExplanation(
                    memory_id=record.id, kind=record.kind.value, scope=record.scope.value,
                    source_type=record.source_type.value, version=record.version,
                    source_session_id=record.source_session_id,
                    source_event_ids=list(record.source_event_ids), ranking=ranking,
                ))
            if explanations:
