...
2. **形状扫描**（兜底）：抓 `sk-` / `AKIA…` / JWT / 长 hex、`Authorization: Bearer …`、
   `api_key=…` 赋值、URL userinfo（`https://user:pass@…`）这类**未配置凭证**的回显面。
...
_KEY_SHAPED = (
    re.compile(r"\b(?:sk|pk|rk|hf|ghp|gho|glpat|xox[baprs])[-_][A-Za-z0-9_\-]{12,}"),
...
_SECRET_ASSIGNMENT = re.compile(
    r"(?i)\b([A-Za-z0-9_]*(?:api[_-]?key|apikey|secret|token|password|passwd|credential)"
...
def credential_values(settings: Any) -> list[str]:
    ...
    字段名猜谜。`GETTER` 缺失（鸭子类型替身）或取值为空/空白 ⇒ 跳过。
    """
    ...
    for name in type(settings).model_fields:
        ...
        if value and value.strip() and len(value) >= MIN_SCANNED_SECRET_LEN:
            values.append(value)
    return values
